...
function classifyGroup(filePath, members) {
  ...
  if (
    ...
    return {
      keep: true,
      reason: `brand family "${firstTokens[0]}" (${members.length} variants in ${[...cats].join(",")})`,
    };
  }
...
for (const [hero, members] of byHero) {
  ...
  if (keep) KEEP.push(entry);
  else BREAK.push(entry);
}
...
const breakProducts = BREAK.flatMap((g) =>
  g.members.map((m) => ({
    rms_id: m.rms_id,
    title: m.title,
    category: m.category,
    current_hero: g.hero_url,
...
lines.push(
  `  KEEP (legit variant families): ${KEEP.length}  → ${KEEP.reduce((s, g) => s + g.member_count, 0)} products`,
);
...
for (const [folder, n] of Object.entries(orphansByFolder)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 15)) {
