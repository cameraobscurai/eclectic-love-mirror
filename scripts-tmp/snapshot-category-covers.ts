...
import {
  ...
  type BrowseGroupId,
} from "../src/lib/collection-browse-groups";
...
for (const id of ORDER) {
  ...
  if (!products.length) {
    console.log("skip empty group:", id);
    continue;
  }
...
for (const j of plan) {
  try {
    ...
    const { error } = await sb.storage
      .from(BUCKET)
      .upload(j.targetPath, buf, { contentType: ct, upsert: true });
    if (error) throw error;
    ok++;
    console.log("OK", j.targetPath, buf.length);
  } catch (e: any) {
    fail++;
