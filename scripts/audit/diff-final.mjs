...
let items = [];
for (const k of Object.keys(cat)) if (Array.isArray(cat[k])) items = items.concat(cat[k]);
...
function classify(p) {
  ...
  else if (cat === "lighting" || cat === "chandeliers" || cat === "candlelight")
    parent = "lighting";
  ...
  switch (parent) {
    ...
    case "lounge-tables":
      ...
      else if (has(t, ["side table", "end table", "accent table", "drink table"]))
        sub = "side-tables";
    ...
    case "cocktail-bar":
      if (has(t, ["cocktail table", "column", "plinth"])) sub = "cocktail-tables";
        ...
        cat === "storage" ||
        has(t, ["cabinet", "credenza", "sideboard", "shelf", "shelving", "etagere"])
      )
        sub = "storage";
    ...
    case "tableware":
      if (has(t, ["platter", "pitcher", "tureen", "server", "tray"])) sub = "serveware";
        ...
        has(t, ["glass", "flute", "goblet", "tumbler", "coupe", "rocks", "wine", "champagne"])
      )
        sub = "glassware";
...
for (const [liveSlug, info] of Object.entries(live.byLiveSlug)) {
  ...
  if (!our) {
    ...
    for (const c of catByTokens) {
      let inter = 0;
      for (const t of lt) if (c.tokens.has(t)) inter++;
  ...
  if (expected.length === 0 || expected.includes(null)) {
    ok++;
    continue;
  }
  if (expected.includes(sub)) {
    ok++;
    continue;
  }
  mismatches.push({
    title: info.liveTitle || info.title,
    live: info.subcategories.join("|"),
    expected: expected.join("|"),
    got: sub,
  });
}
console.log(
  `matched: ${ok + mismatches.length}, missing-from-catalog: ${missing}, TRUE mismatches: ${mismatches.length}`,
);
for (const m of mismatches)
  console.log(
    `  · ${m.title.padEnd(45)} live=${m.live.padEnd(20)} expected=${m.expected.padEnd(18)} got=${m.got || "(none)"}`,
  );
writeFileSync(
  "scripts/audit/live-vs-runtime-final.json",
  JSON.stringify({ mismatches, missingCount: missing }, null, 2),
);
