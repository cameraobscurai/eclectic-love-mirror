...
const pieceOf = (fn) => {
  const base = fn.replace(/\.[^.]+$/, "").trim();
  return base
    .replace(new RegExp("^" + stemOf(fn).toUpperCase() + "\\s*", "i"), "")
    .trim()
    .toLowerCase();
};
...
for (const r of twRows) {
  ...
  for (const f of files) {
    ...
    if (titleNumeric && piece.startsWith(titleNumeric)) {
      hero = f;
      break;
    }
  ...
  if (hero) {
    twResolved++;
    twResolvedRows.push({ id: r.id, title: r.title, file: hero.raw_filename });
...
const fileTokens = (f) =>
  norm(f.raw_filename.replace(/\.[^.]+$/, ""))
    .split(" ")
    .filter((t) => t && !["set", "throws"].includes(t));
...
for (const r of thRows) {
  ...
  if (best) {
    thResolved++;
    thHits.push({ id: r.id, title: r.title, file: best.raw_filename, score: bestScore.toFixed(2) });
...
for (const r of pRows) {
  const rt = norm(r.title)
    .split(" ")
    .filter((t) => t && t !== "pillow" && t !== "lumbar" && t !== "oversize" && t !== "mini");
  let best = null,
    bestScore = 0;
  for (const f of pFiles) {
    const ft = fileTokens(f).filter(
      (t) => t !== "pillow" && t !== "pillows" && t !== "lumbar" && t !== "oversize",
    );
    const overlap = rt.filter((t) => ft.includes(t)).length;
    const score = overlap / Math.max(rt.length, 1);
    // pillows allowed lower threshold per owner: 2 token overlap OR (1 token + score>=0.5)
    if ((overlap >= 2 || (overlap >= 1 && score >= 0.5)) && score > bestScore) {
      best = f;
      bestScore = score;
    }
  }
  if (best) {
    pResolved++;
    pHits.push({ id: r.id, title: r.title, file: best.raw_filename, score: bestScore.toFixed(2) });
  } else pMisses.push(r);
}
...
fs.writeFileSync(
  "scripts-tmp/soft-tableware-hits.json",
  JSON.stringify({ tableware: twResolvedRows, throws: thHits, pillows: pHits }, null, 2),
);
fs.writeFileSync(
  "scripts-tmp/soft-tableware-misses.json",
  JSON.stringify(
    {
      tableware: twUnresolvedRows.map((r) => ({ id: r.id, title: r.title })),
      throws: thMisses.map((r) => ({ id: r.id, title: r.title })),
      pillows: pMisses.map((r) => ({ id: r.id, title: r.title })),
    },
    null,
    2,
  ),
