...
for (let i = 0; i < all.length; i += B) {
  ...
  try {
    ...
    part.forEach((p, k) => (out[p] = res[k] || {}));
    console.log(`  ${i + part.length}/${all.length}`);
  } catch (e) {
    console.log(`  fail ${i}: ${e.message.slice(0, 150)}`);
    part.forEach((p) => (out[p] = { category: "other" }));
  }
}
...
for (const [path, t] of Object.entries(out)) {
  ...
  const slug =
    String(t.object || "item")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 40) || "item";
  let target = `${cat}/${slug}.png`;
  const n = (used.get(target) || 0) + 1;
  used.set(target, n);
