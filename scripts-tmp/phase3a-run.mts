...
async function main() {
  ...
  const { data: existing } = await sb.from("scraped_product_pages_markdown").select("url");
  const done = new Set((existing ?? []).map((r) => r.url));
  const todo = manifest!.filter((m) => !done.has(m.url));
  console.log(
    `[manifest] total=${manifest!.length} already_scraped=${done.size} todo=${todo.length}`,
  );
  if (todo.length === 0) {
  ...
  for (const item of todo) {
    try {
      ...
      if (completed % 25 === 0 || completed === todo.length) {
        console.log(
          `[progress] ${completed}/${todo.length} ok=${parsed_ok} low=${parsed_low} err=${errors} 429=${rate429} credits=${credits}`,
        );
        await sb
          .from("scrape_runs")
