...
for (const [root, lives] of liveByRoot) {
  for (const l of lives) {
    ...
    if (totalRows >= 2) {
      liveCoveredByMulti++;
      liveCardSizes.push([l.title, totalRows]);
...
const out = {
  ...
  derivedGroups: {
    ...
    largestGroups: multiGroups
      ...
      .map((g) => ({
        ...
        sample: g.items.slice(0, 3).map((i) => i.title),
      })),
  },
  liveCoverage: {
  ...
  projectedPublicCards: singletons.length + multiGroups.length,
};

fs.writeFileSync(
  path.join(ROOT, "scripts/audit/product-groups.json"),
...
console.log(
  `  → delta:                     ${out.projectedPublicCards - out.liveCoverage.liveTotal}`,
);
...
for (const g of out.derivedGroups.largestGroups.slice(0, 12)) {
  console.log(
    `  ${String(g.count).padStart(2)}× ${g.root.padEnd(14)} [${g.slug}]   e.g. ${g.sample.join(" | ")}`,
  );
}
