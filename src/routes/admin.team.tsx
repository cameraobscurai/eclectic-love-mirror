...
export const Route = createFileRoute("/admin/team")({
  ...
  head: () => ({
    meta: [{ title: "Team · Admin" }, { name: "robots", content: "noindex, nofollow" }],
  }),
  component: TeamInner,
});
    ...
    <div className="min-h-[calc(100vh-3rem)] bg-cream text-charcoal">
      <div className="px-6 lg:px-12 pt-8 pb-24 max-w-[1100px] mx-auto">
        ...
        <section className="mb-10 border border-charcoal/10 p-5">
          <h2 className="text-[10px] uppercase tracking-[0.26em] text-charcoal/50 mb-3">
            Invite staff
          </h2>
        ...
        {err && (
          <div className="mb-4 p-3 border border-red-300 bg-red-50 text-[12px] text-red-800">
            {err}
          </div>
        )}
        ...
        <div className="border border-charcoal/10">
          <table className="w-full text-[12px]">
            ...
            <tbody>
              {loading && (
                <tr>
                  <td colSpan={4} className="px-3 py-10 text-center text-charcoal/40">
                    Loading…
                  </td>
                </tr>
              )}
                  ...
                  return (
                    <tr key={m.id} className="border-b border-charcoal/5">
                      ...
                      <td className="px-3 py-2">
                        {isAdmin && (
                          <span className="inline-block mr-1 px-1.5 py-0.5 text-[10px] uppercase tracking-[0.14em] bg-charcoal text-cream">
                            Admin
                          </span>
                        )}
                        {isStaff && (
                          <span className="inline-block mr-1 px-1.5 py-0.5 text-[10px] uppercase tracking-[0.14em] border border-charcoal/40">
                            Staff
                          </span>
                        )}
