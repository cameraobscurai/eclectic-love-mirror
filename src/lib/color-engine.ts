...
export function generateInsights(palette: ColorInfo[], tones: ToneAnalysis): DesignInsight[] {
  ...
  if (hueSpread < 30)
    ins.push({
      icon: "🎨",
      title: "Monochromatic",
      text: "The palette stays in a tight hue range, creating a cohesive, harmonious feel.",
    });
  else if (hueSpread < 80)
    ins.push({
      icon: "🎨",
      title: "Analogous harmony",
      text: "Hues are closely related, suggesting a calm and unified aesthetic.",
    });
  else
    ins.push({
      icon: "🎨",
      title: "Contrast palette",
      text: "A wide hue range creates visual tension and energy.",
    });

  if (tones.warm > 55)
    ins.push({
      icon: "🌅",
      title: `Warm-dominant (${tones.warm}%)`,
      text: "Oranges, reds and yellows suggest comfort, earthiness, or energy.",
    });
  else if (tones.cool > 55)
    ins.push({
      icon: "🧊",
      title: `Cool-dominant (${tones.cool}%)`,
      text: "Blues and greens evoke calm, freshness, or sophistication.",
    });
  else
    ins.push({
      icon: "⚖️",
      title: "Balanced temperature",
      text: "Warm and cool tones balance well, giving versatility.",
    });
  ...
  if (tones.light > 65)
    ins.push({
      icon: "☀️",
      title: "Light & airy",
      text: "High luminosity suggests a bright, open, Scandinavian or minimalist direction.",
    });
  else if (tones.dark > 55)
    ins.push({
      icon: "🌑",
      title: "Moody & dramatic",
      text: "Deep tones suggest a luxurious, editorial, or industrial aesthetic.",
    });
  else
    ins.push({
      icon: "🌗",
      title: "Balanced luminosity",
      text: "A mix of lights and darks creates depth and layering.",
    });

  if (tones.saturated > 60)
    ins.push({
      icon: "💎",
      title: "High saturation",
      text: "Vivid hues create boldness. Use accent walls, art, or upholstery sparingly.",
    });
  else if (tones.muted > 70)
    ins.push({
      icon: "🪵",
      title: "Muted & desaturated",
      text: "Dusty, earthy tones align with organic, artisan, or biophilic interiors.",
    });
  ...
  ins.push({
    ...
    text: `Use ${hex0} as the base wall or floor, and ${hex1} as the dominant furniture or trim tone.`,
  });
  ...
  ins.push({
    ...
    text: `${unique.join(", ")} tones commonly appear in ${styleHint(tones)} interiors.`,
  });
