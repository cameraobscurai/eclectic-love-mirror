 ...
 */
const OWNER_DECLARED_COLUMNS = ["collection_slug", "category_slug", "taxonomy_review", "images"];
...
const existing = new Set();
{
  ...
  for (let from = 0; ; from += PAGE) {
    ...
    if (error) {
      console.error("existing-id fetch error", error);
      process.exit(1);
    }
...
if (DRY_RUN) {
  ...
  console.log(
    "[dry-run] columns never written on existing rows:",
    OWNER_DECLARED_COLUMNS.join(", "),
  );
  process.exit(0);
}
...
for (const rec of toUpdate) {
  ...
  if (error) {
    console.error("update error", rec.rms_id, error);
    process.exit(1);
  }
  updated += 1;
