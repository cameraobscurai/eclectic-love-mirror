...
const files = JSON.parse(readFileSync("/tmp/incoming-files.json", "utf8")).filter(
  (f) => f.name && !f.name.startsWith(".") && /\.(png|jpe?g|webp)$/i.test(f.name),
);
...
for (const g of groups.values()) {
  g.files.sort((a, b) => {
    const ai = a.match(/\s(\d+)\.[^.]+$/);
    const bi = b.match(/\s(\d+)\.[^.]+$/);
    const an = ai ? parseInt(ai[1]) : -1;
    const bn = bi ? parseInt(bi[1]) : -1;
    return an - bn;
  });
}
...
for (const g of groups.values()) {
  ...
  if (inCat.length === 1) pick = inCat[0];
    ...
    ambiguous.push({ ...g, candidates: inCat });
    continue;
  } else if (candidates.length === 1)
    pick = candidates[0]; // category mismatch but unique title
  else if (candidates.length > 1) {
    ambiguous.push({ ...g, candidates });
    continue;
  }
  ...
  if (pick) {
    matched.push({
      ...
      product: {
        ...
        rms_id: pick.rms_id,
        currentImages: pick.images?.length || 0,
      },
...
for (const [f, s] of Object.entries(folderStats).sort()) {
  console.log(
    f.padEnd(28),
    String(s.matched).padStart(5),
    String(s.ambiguous).padStart(4),
    String(s.set).padStart(4),
    String(s.orphan).padStart(5),
  );
}

console.log("\nSAMPLE ORPHANS (first 30):");
for (const o of orphans.slice(0, 30))
  console.log(`  [${o.folder}] ${o.files[0]}  (norm: ${o.base})`);

console.log("\nSAMPLE AMBIGUOUS (first 15):");
for (const a of ambiguous.slice(0, 15)) {
  console.log(
    `  [${a.folder}] ${a.files[0]} → ${a.candidates.map((c) => `${c.title} (${c.category})`).join(" | ")}`,
  );
}
...
writeFileSync(
  "/tmp/incoming-match-report.json",
  JSON.stringify(
    {
      matched,
      ambiguous,
