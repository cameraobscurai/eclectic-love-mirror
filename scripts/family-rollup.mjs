...
export function rollupFamilies(
  ...
  for (const g of forcedGroups || []) {
    ...
    const liveSlug = lp
      ...
      : norm(g.live)
          ...
          .replace(/^-+|-+$/g, "");
    if (!lp)
      console.warn("[rollup] forced group live tile not in snapshot, using owner title:", g.live);
    for (const rmsId of g.rms || []) {
      forcedByRms.set(String(rmsId), {
        key: "live:" + liveSlug,
        source: lp ? "forced" : "forced-owner",
        familyTitle,
        liveSlug,
      });
    }
  ...
  if (forcedByRms.size)
    console.log(`[rollup] ${forcedByRms.size} rms_ids assigned by forced groups`);
  ...
  function familyKeyForRms(p) {
    ...
    if (liveByKey.has(k))
      return {
        key: "live:" + liveByKey.get(k).slug,
        source: "exact",
        familyTitle: p.title,
        liveSlug: liveByKey.get(k).slug,
      };
    ...
    for (const lp of liveProducts) {
      ...
      if (core.every((t) => rmsTokSet.has(t))) {
        if (!bestSubset || core.length > bestSubsetCoreLen) {
          bestSubset = lp;
          bestSubsetCoreLen = core.length;
        }
      }
    }
    if (bestSubset)
      return {
        key: "live:" + bestSubset.slug,
        source: "tokens-subset",
        familyTitle: bestSubset.title,
        liveSlug: bestSubset.slug,
      };
    ...
    if (firstTok && hasVariantNoun(p.title)) {
      ...
      if (candidates) {
        ...
        if (familyCards.length === 1) {
          return {
            key: "live:" + familyCards[0].slug,
            source: "family-first-token",
            familyTitle: familyCards[0].title,
            liveSlug: familyCards[0].slug,
          };
        }
        if (familyCards.length > 1) {
          ...
          if (refined.length === 1)
            return {
              key: "live:" + refined[0].slug,
              source: "family-first-token-refined",
              familyTitle: refined[0].title,
              liveSlug: refined[0].slug,
            };
        }
      }
    }
  ...
  for (const [key, g] of groups) {
    ...
    const withMostImages = [...sorted].sort(
    ...
    const isSetImage = (img) => {
      ...
      if (
        familyLead &&
        new RegExp(
          `(?:^|[\\s._+-])${familyLead}[\\s._+-]+(bowls|trays|plates|goblets)(?:[\\s._+-]|\\.|$)`,
          "i",
        ).test(key)
      )
        return true;
    ...
    const scoreVariantImage = (img, member) => {
      ...
      const tokens = wordTokens(member.title).filter(
        (t) => !family.has(t) || VARIANT_NOUN_STEMS.has(t),
      );
    ...
    if (
      (withMostImages.categorySlug === "tableware" ||
        withMostImages.categorySlug === "serveware") &&
      ...
      const coverScore = (img) => {
        ...
        if (
          /\bplates?|stoneware|flatware|glassware\b/.test(titleKey) &&
          /\b(set|collection)\b/.test(key)
        )
          score += 15;
      ...
      if (setIdx < 0)
        setIdx = mergedImages.findIndex((img) => {
          const url = typeof img === "string" ? img : img.url || img.src || "";
    ...
    const family = {
      ...
      variants: sorted.map((m) => {
        ...
        const imageUrl = variantImg
          ? typeof variantImg === "string"
            ? variantImg
            : variantImg.url || variantImg.src || null
          : null;
        return {
          id: m.id,
          title: m.title,
