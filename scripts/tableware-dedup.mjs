...
const rows = readFileSync("/tmp/tableware.tsv", "utf8")
  .trim()
  .split("\n")
  .map((l) => {
...
for (const r of rows) {
  ...
  for (const u of r.images) {
    ...
    if (overlap) keep.push(u);
    else drop.push({ url: u, fnPieces });
  }
  if (drop.length) nameFindings.push({ ...r, ownPieces, keep, drop });
}

console.log(
  `name-based: ${nameFindings.length} rows would change, ${nameFindings.reduce((s, f) => s + f.drop.length, 0)} images dropped`,
);
...
await Promise.all(
  Array.from({ length: 24 }, async () => {
    while (cur < tasks.length) {
      const i = cur++;
      results[i] = await dhash(tasks[i].u);
      done++;
...
console.log(
  `total images dropped: ${finalPlan.reduce((s, f) => s + (f.originalCount - f.finalCount), 0)}`,
);
...
// preview top changes
console.log("\nsample (first 10):");
for (const p of finalPlan.slice(0, 10))
  console.log(`  ${p.title}: ${p.originalCount} → ${p.finalCount}`);
