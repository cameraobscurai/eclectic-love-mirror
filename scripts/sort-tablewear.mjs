...
const files = (
  await c.query(`select name from storage.objects where bucket_id='${BUCKET}' order by name`)
).rows.map((r) => r.name);

const items = (
  await c.query(`select rms_id, title, images from inventory_items where category='tableware'`)
).rows;
...
function parse(file) {
  ...
  let family, rest;
  if (/^MIDAS VIDAL\b/i.test(base)) {
    family = "MIDAS VIDAL";
    rest = base.slice(11).trim();
  } else {
    const i = base.indexOf(" ");
    family = i < 0 ? base : base.slice(0, i);
    rest = i < 0 ? "" : base.slice(i + 1).trim();
  }
  return { family, rest };
}
...
for (const f of files) {
  ...
  if (isSetFile(rest, family)) {
    ...
    plan.push({
      ...
      targets: familyItems.map((i) => i.rms_id),
    });
    continue;
  }
  ...
  plan.push({
    ...
    targets: matches.map((m) => `${m.rms_id}|${m.title}`),
  });
}
...
let total_adds = 0,
  total_removes = 0;
for (const [, ops] of itemUpdates) {
  total_adds += ops.add.size;
  total_removes += ops.remove.size;
}
console.log(`\n=== DB CHANGES ===`);
...
for (const [rms_id, ops] of itemUpdates) {
  ...
  console.log(
    `  remove:  ${[...ops.remove].map((u) => u.split("/").pop()).join(", ") || "(none)"}`,
  );
}
...
import fs from "fs";
fs.writeFileSync(
  "/tmp/tablewear-plan.json",
  JSON.stringify(
    {
      plan,
      orphans,
      itemUpdates: [...itemUpdates].map(([rms_id, ops]) => ({
        rms_id,
        add: [...ops.add],
        remove: [...ops.remove],
      })),
