    ...
    try {
      ...
      if (route.startsWith("/contact")) {
        ...
        const [response] = await Promise.all([
          page
            .waitForResponse(
              (res) => res.url().includes("style-brief") || res.url().includes("functions/v1/"),
              { timeout: 10000 },
            )
            .catch(() => null),
          page
            .click('button[type="submit"]')
            .catch(() => console.log("Submit button click failed")),
        ]);
      ...
      results[route] = {
        pass:
          errors.filter((e) => !e.includes("ResizeObserver")).length === 0 &&
