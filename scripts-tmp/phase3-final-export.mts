...
async function main() {
  ...
  for (const p of products) {
    ...
    productRows.push([
      ...
      p.product_slug,
      title,
      p.product_title_original,
      p.product_title_normalized,
      p.is_custom_order_co,
      p.description,
      p.dimensions,
      p.stocked_quantity,
      p.material_notes,
      ...
      p.quantity_selector_present,
      p.variant_selector_present,
      p.previous_product_url,
      p.next_product_url,
      p.related_product_urls,
      p.breadcrumb,
      p.repaired_by_llm,
      p.extraction_method,
      p.final_confidence,
  ...
  for (const im of images) {
    const owner =
      productByUrl.get(im.source_page_url) ??
  ...
  await writeFile(
    join(OUT_DIR, "phase3_manual_review_queue.csv"),
  ...
  const catHeaders = [
    ...
    "products_with_stocked_quantity",
    "custom_order_count",
    "avg_confidence",
    "manual_review_count",
  ];
  ...
  for (const p of products) {
    ...
    let s = catMap.get(k);
    if (!s) {
      s = { count: 0, img: 0, noimg: 0, dim: 0, qty: 0, co: 0, confSum: 0, confN: 0, review: 0 };
      catMap.set(k, s);
    }
    s.count++;
    if (p.hero_image_url) s.img++;
    else s.noimg++;
    if (p.dimensions) s.dim++;
    if (p.stocked_quantity) s.qty++;
    if (p.is_custom_order_co) s.co++;
    if (p.final_confidence != null) {
      s.confSum += Number(p.final_confidence);
      s.confN++;
    }
  ...
  for (const [slug, s] of [...catMap.entries()].sort()) {
    catRows.push([
      ...
      s.qty,
      s.co,
      s.confN ? (s.confSum / s.confN).toFixed(3) : "",
      s.review,
    ]);
  }
