...
for (const url of urls) {
  ...
  if (existsSync(cachePath)) {
    ...
    writeFileSync(
      cachePath,
      JSON.stringify({ markdown: res.markdown, metadata: res.metadata, links: res.links }),
    );
  ...
  if (p.parse_confidence < 0.85)
    lowConf.push({ url, conf: p.parse_confidence, missing: p.missing_fields });
  console.log(
...
console.log(
  "needs reextract:",
  `${needReextract}/${urls.length} (${Math.round((needReextract / urls.length) * 100)}%)`,
);
