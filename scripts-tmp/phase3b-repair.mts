    ...
    parameters: {
      ...
      properties: {
        ...
        product_title_normalized: {
          type: ["string", "null"],
          description: "Title with site suffix (— Event Design & Decor | Eclectic Hive) stripped.",
        },
        description: { type: ["string", "null"] },
        dimensions: {
          type: ["string", "null"],
          description: 'e.g. 18"W x 18"H. null if not visible.',
        },
        stocked_quantity: {
          type: ["string", "null"],
          description: 'e.g. "12" or "*". null if not visible.',
        },
        image_urls: {
          type: "array",
          items: { type: "string" },
          description:
            "Product image URLs in display order. Only Squarespace CDN images. Exclude logos/footer chrome.",
        },
        ...
        confidence: {
          type: "number",
          description: "0-1 self-assessed confidence in the extraction.",
        },
        repaired_fields: {
          type: "array",
          items: { type: "string" },
          description: "Field names you successfully extracted.",
        },
...
async function main() {
  ...
  for (const c of cands!) {
    try {
      ...
      const image_urls: string[] = Array.isArray(args.image_urls)
        ? args.image_urls.filter(Boolean)
        : [];
      const inferred_filenames: string[] = image_urls.map(inferFilename).map((s) => s ?? "");
      const alt_texts: string[] = Array.isArray(args.alt_texts)
        ? args.alt_texts.slice(0, image_urls.length)
        : [];
    ...
    if ((ok + fail) % 5 === 0)
      console.log(`[progress] ${ok + fail}/${cands!.length} ok=${ok} fail=${fail}`);
  }
