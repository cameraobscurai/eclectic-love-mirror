...
function extractTitle(html: string): string | null {
  ...
  const m =
    html.match(/<h1[^>]*ProductItem-details-title[^>]*>([\s\S]*?)<\/h1>/i) ??
...
async function main() {
  // Load resume state
  let state: { detailUrls: Record<string, string[]>; products: Product[] } = {
    detailUrls: {},
    products: [],
  };
  if (fs.existsSync(STATE)) {
    ...
    console.log(
      `[resume] cats=${Object.keys(state.detailUrls).length} products=${state.products.length}`,
    );
  }
  ...
  for (const cat of CATEGORY_PATHS) {
    ...
    try {
      ...
      const detail = [
        ...new Set(links.filter((l) => typeof l === "string" && l.startsWith(prefix))),
      ];
