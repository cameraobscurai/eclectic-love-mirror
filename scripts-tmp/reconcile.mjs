...
for (const r of publicRows) {
  ...
  const isSafeFilenameSqs =
    b.source === "filename+squarespace" && (conf === "exact" || conf === "contains");
  ...
  if ((isSafeFilename || isSafeFilenameSqs || isSafeSqsExact) && variantOk) {
    APPLY_SAFE.push({
      ...
      product_name: r.title,
      category: r.category,
      image_source: b.source,
      primary_image_url: b.primary,
    ...
    REVIEW.push({
      ...
      source: b.source,
      reason: !variantOk
        ? "variant-family bind: RMS title shares no distinctive token with site title"
...
for (const r of publicRows) {
  ...
  if (f && /astaire/i.test(f))
    astaireRms.push({ rms_id: r.rms_id, title: r.title, csv_filename: f });
}
...
const monroeManifest = {
  ...
  squarespace_products: monroeSite.map((sp) => ({
    ...
    image_count: sp.cdn_image_urls.length,
    hero: sp.cdn_image_urls[0],
  })),
  recommended_bindings: monroeRms.map((r) => {
    ...
    const storageMatch = want
      ? monroeStorage.find((p) =>
          normalizeKey(p).replace(/\s+/g, " ").includes(want.replace(/'/g, "").toLowerCase()),
        )
      : null;
...
const rmsIndex = publicRows.map((r) => ({
  rms_id: r.rms_id,
  title: r.title,
  key: normalizeKey(r.title),
  tokens: new Set(normalizeKey(r.title).split(" ")),
}));
function closestRms(siteTitle) {
  ...
  return best && bestScore >= 2
    ? { rms_id: best.rms_id, title: best.title, score: bestScore }
    : null;
}
const unmatchedSiteAnalysis = unmatchedSiteList.map((sp) => ({
  site_title: sp.title,
  site_category_path: sp.category_path,
  site_slug: sp.slug,
  detail_url: sp.detail_url,
...
fs.writeFileSync(
  OUT("reconcile-unmatched-site.json"),
  JSON.stringify(unmatchedSiteAnalysis, null, 2),
);

// --- recompute coverage AFTER Monroe patch (assuming all monroe recommended bindings apply) ---
const monroePatchable = monroeManifest.recommended_bindings.filter(
  (b) => b.recommended_storage,
).length;
...
const imagelessRms = REVIEW.filter(
  (r) => !r.closest_storage_candidate && !r.closest_squarespace_candidate,
).length;
...
lines.push(
  "   of which Monroe/Astaire cluster:                 " +
    CONFLICTS.filter((c) => /monroe/i.test(c.product_name)).length,
);
...
lines.push(
  "Image-less after Monroe patch:    " +
    imagelessAfterMonroe +
    "  (Monroe patchable: " +
    monroePatchable +
    ")",
);
...
for (const [cat, n] of Object.entries(reviewByCat)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 10)) {
  lines.push(`   ${cat}: ${n}`);
}
...
for (const a of APPLY_SAFE.slice(0, 5))
  lines.push(
    `   ${a.product_id} "${a.product_name}" [${a.category}] src=${a.image_source} conf=${a.confidence} gallery=${a.gallery_image_count}`,
  );
lines.push("");
...
for (const c of CONFLICTS.slice(0, 5)) {
  lines.push(
    `   ${c.product_id} "${c.product_name}" csv="${c.csv_original_filename}" → ${c.recommended_resolution}`,
  );
}
lines.push("");
...
for (const u of unmatchedSiteAnalysis.slice(0, 5)) {
  lines.push(
    `   "${u.site_title}" (${u.site_category_path}) closest=${u.closest_rms_candidate ? `${u.closest_rms_candidate.rms_id}/"${u.closest_rms_candidate.title}" score=${u.closest_rms_candidate.score}` : "(none)"}`,
  );
}
lines.push("");
...
lines.push(
  `Recommended bindings with storage match: ${monroePatchable} of ${monroeManifest.rms_rows.length}`,
);
