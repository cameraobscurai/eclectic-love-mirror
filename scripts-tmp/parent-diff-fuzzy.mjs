...
const productParent = (p) =>
  (p.liveCategory && LIVE_CAT_TO_PARENT[p.liveCategory]) || CAT_TO_PARENT[p.categorySlug] || null;
...
for (const [slug, parent] of Object.entries(FEED_TO_PARENT)) {
  ...
  report.push({
    ...
    maybe: maybe.length,
    realGap_liveOnly: realGap.length,
    realGap_oursOnly: ourOrphans.length,
    maybePairs: maybe.map((m) => ({
      live: m.live.title,
      our: m.our.title,
...
for (const r of report) {
  console.log(
    ...
    String(r.ours).padStart(4),
    String(r.matched).padStart(4),
    String(r.maybe).padStart(4),
    String(r.realGap_liveOnly).padStart(6),
    String(r.realGap_oursOnly).padStart(7),
  );
}
