...
for (const x of r.blanks_HIGH) {
  ...
  const { data: cur, error: e1 } = await sb
    ...
    .single();
  if (e1) {
    fail.push({ rms: x.rms, err: e1.message });
    continue;
  }
  if ((cur.images || []).length > 0) {
    fail.push({ rms: x.rms, skip: "already has images" });
    continue;
  }
  const { error } = await sb
    .from("inventory_items")
    .update({ images: x.urls, updated_at: new Date().toISOString() })
    .eq("rms_id", x.rms);
  if (error) fail.push({ rms: x.rms, err: error.message });
  else okBlank++;
}
...
fs.writeFileSync(
  "scripts-tmp/site-json-apply-result.json",
  JSON.stringify({ okBlank, okGal, fail }, null, 2),
);
