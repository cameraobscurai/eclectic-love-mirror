...
export const Route = createFileRoute("/admin/colors")({
  ...
  component: ColorsQA,
  head: () => ({
    meta: [{ title: "Color QA — Admin" }, { name: "robots", content: "noindex, nofollow" }],
  }),
});
...
const FAMILY_SWATCH: Record<string, string> = {
  ...
  purple: "#6e4d8a",
  pink: "#d29eb0",
  "metallic-warm": "#b89968",
  "metallic-cool": "#a8acb3",
  multi: "linear-gradient(90deg,#a83232,#d8c34a,#3a567a)",
};
  ...
  return (
    <div className="min-h-screen bg-cream text-charcoal p-6">
      ...
      {data && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6 text-sm">
          ...
          <Stat
            label="Untagged"
            value={data.untagged}
            accent={data.untagged > 0 ? "warn" : undefined}
          />
          <Stat
            label="Needs review"
            value={data.needsReview}
            accent={data.needsReview > 0 ? "warn" : undefined}
          />
      ...
      <div className="flex flex-wrap gap-3 mb-4 text-sm">
        ...
        <button onClick={refresh} className="border border-charcoal/20 px-3 py-1">
          Refresh
        </button>
      </div>
        ...
        {(data?.rows ?? []).map((r) => (
          ...
          >
            ...
            <div className="p-2">
              ...
              <div className="text-[11px] truncate" title={r.title}>
                {r.title}
              </div>
      ...
      {selected && (
        <div className="fixed top-0 right-0 w-full sm:w-[420px] h-full bg-white border-l border-charcoal/20 shadow-xl overflow-y-auto p-5 z-50">
          <button onClick={() => setSelected(null)} className="text-xs uppercase opacity-60 mb-3">
            ← Close
          </button>
          <div className="aspect-square bg-charcoal/5 mb-3 overflow-hidden">
            {selected.hero && (
              <img src={selected.hero} alt="" className="w-full h-full object-cover" />
            )}
          </div>
            ...
            onLockToggle={async () => {
              ...
              try {
                const result = await runAdminMutation(
                  () =>
                    setColorLocked({
                      data: { rms_id: selected.rms_id, locked: !selected.color_locked },
                    }),
...
function Stat({ label, value, accent }: { label: string; value: number; accent?: "warn" }) {
  return (
    <div className="border border-charcoal/10 p-3">
      ...
      <div
        className={["text-2xl font-display", accent === "warn" ? "text-orange-700" : ""].join(" ")}
      >
        {value}
      </div>
    </div>
  ...
  return (
    <div className="border border-charcoal/10 px-2 py-1">
      ...
      <div className="flex items-center gap-1">
        {swatch && (
          <span
            className="inline-block w-3 h-3 border border-charcoal/20"
...
function OverridePanel({
  ...
  const [family, setFamily] = useState<(typeof FAMILIES)[number]>(
    (row.color_family as (typeof FAMILIES)[number]) ?? "brown",
  );
  const [temp, setTemp] = useState<"warm" | "neutral" | "cool">(
    (row.color_temperature as "warm" | "neutral" | "cool") ?? "neutral",
  );
  ...
  return (
    <div className="border-t border-charcoal/10 pt-3 mt-3">
          ...
          <button
            ...
            style={
              family === f
                ? undefined
                : {
                    background: FAMILY_SWATCH[f]?.startsWith("linear")
                      ? undefined
                      : FAMILY_SWATCH[f] + "33",
                  }
            }
      ...
      <div className="flex gap-2 mb-3 text-xs">
        {(["warm", "neutral", "cool"] as const).map((t) => (
          <button
            ...
            onClick={() => setTemp(t)}
            className={[
              "flex-1 py-1 border uppercase",
              temp === t ? "border-charcoal bg-charcoal text-white" : "border-charcoal/15",
            ].join(" ")}
          >
            {t}
          </button>
      ...
      <textarea
        ...
        className="w-full border border-charcoal/20 px-2 py-1 text-xs mb-3"
        rows={2}
      />
      ...
      <div className="flex gap-2">
        <button
          disabled={busy}
          onClick={() => onSave({ hex, family, temperature: temp, notes, lock: true })}
