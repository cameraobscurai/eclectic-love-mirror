...
if (!ghostIds.length) {
  console.error("no ghost ids found in docs/taxonomy-open-questions.md");
  process.exit(1);
}
...
for (const ghostId of ghostIds) {
  ...
  if (!ruling) {
    reports.push({ ghostId, why: "no workbook row for this id" });
    continue;
  }
  if (ruling.unknown || ruling.blank) {
    reports.push({
      ghostId,
      title: ruling.title,
      why: ruling.blank ? "blank ruling" : "off-vocabulary ruling",
    });
    continue;
  }
  const matches = byNormTitle.get(normTitle(ruling.title)) ?? [];
  if (matches.length === 0) {
    reports.push({ ghostId, title: ruling.title, why: "zero title matches in db" });
    continue;
  }
  if (matches.length > 1) {
    reports.push({
      ghostId,
      title: ruling.title,
      why: `${matches.length} title matches — ambiguous`,
      matches: matches.map((m) => m.rms_id),
    });
    continue;
  }
  const row = matches[0];
  ...
  if (!eligible) {
    reports.push({
      ...
      title: ruling.title,
      why:
        `db row (rms_id ${row.rms_id ?? "—"}) is already assigned ` +
        `${row.collection_slug}/${row.category_slug}` +
        (row.collection_slug === ruling.collection_slug &&
        row.category_slug === ruling.category_slug
  ...
  writes.push({
    ...
    collection_slug: ruling.collection_slug,
    category_slug: ruling.category_slug,
    wasUnassigned: unassigned,
    valuesUnchanged: already,
    source,
  });
}
...
const md = [
  ...
  "| --- | --- | --- | --- | --- | --- |",
  ...writes.map(
    (w) => `| ${w.ghostId} | ${w.rmsId} | ${w.title} | ${w.from} | ${w.to} | ${w.source ?? "—"} |`,
  ),
  "",
  ...
  ...(reports.length
    ? reports.map(
        (r) =>
          `- **${r.ghostId}** ${r.title ? `(${r.title})` : ""} — ${r.why}${r.matches ? ` [${r.matches.join(", ")}]` : ""}`,
      )
    : ["- none"]),
  "",
].join("\n");
...
for (const w of writes)
  console.log(`  ${w.ghostId} -> ${w.rmsId} "${w.title}"  ${w.from} => ${w.to}`);
...
for (const w of writes) {
  const { error } = await sb
    ...
    .eq("id", w.id);
  if (error) {
    console.error(`  FAIL ${w.rmsId}: ${error.message}`);
    fail++;
  } else ok++;
}
