...
export const SceneBrief: React.FC = () => {
  ...
  const camProgress = interpolate(frame, [0, SCENE_LEN - 30], [0, 1], {
  ...
  const liftP = interpolate(frame, [T_LIFT, SCENE_LEN], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  ...
  return (
    <AbsoluteFill
      style={{
        transform: `translateY(${liftY}px) scale(${liftScale})`,
    ...
    >
      ...
      >
        ...
        >
          <Word frame={frame} at={T_WORD1} fps={fps}>
            The{" "}
          </Word>
          <Word frame={frame} at={T_WORD2} fps={fps} italic>
            Ridgeline
          </Word>
          {"\n"}
          <Word frame={frame} at={T_WORD3} fps={fps}>
            Dinner
          </Word>
        ...
        <div style={{ position: "absolute", left: 0, right: 0, top: ROWS_TOP }}>
          {[
            ...
            const labelOp = interpolate(frame, [rowAt, rowAt + 10], [0, 0.55], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
            });
            const typeProgress = interpolate(frame, [rowAt + 4, rowAt + 22], [0, 1], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
              easing: Easing.bezier(0.4, 0, 0.2, 1),
            });
            const ruleOp = interpolate(frame, [rowAt + 16, rowAt + 26], [0, 1], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
            });
            return (
              <div
                ...
                style={{
                  ...
                  borderBottomColor:
                    i === arr.length - 1 ? "transparent" : `rgba(26,26,26,${0.1 * ruleOp})`,
                  height: ROW_H,
                  alignItems: "center",
                }}
              >
          ...
          >
            ...
            >
              <Ticker
                frame={frame}
                startAt={T_PALETTE}
                stepFrames={SW_STAGGER}
                max={REAL_PALETTE.length}
                pad2
              />{" "}
              Tones
            </span>
        ...
        >
          {REAL_PALETTE.map((sw, i) => {
            const at = T_HEX + i * 2;
            const op = interpolate(frame, [at, at + 10], [0, 0.5], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
            });
            return (
        ...
        >
          {PRODUCTS.map((p, i) => {
            ...
            const flash = interpolate(frame, [at, at + 4, at + 16], [0, 1, 0], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
            });
            const borderColor = `rgba(26,26,26,${0.1 + flash * 0.7})`;
            return (
              <div
                key={i}
                style={{
        ...
        {/* SIGNATURE — handwrite */}
        {(() => {
          const sigProgress = interpolate(frame, [T_SIGN, T_SIGN + 28], [0, 1], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
            easing: Easing.bezier(0.4, 0, 0.2, 1),
          });
          const pillBase = interpolate(frame, [T_SIGN + 20, T_SIGN + 30], [0, 0.55], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          });
          const pillPulse = interpolate(
            frame,
            [SCENE_LEN - 40, SCENE_LEN - 30, SCENE_LEN - 20],
            [0, 1, 0.5],
              ...
              <div style={{ overflow: "hidden" }}>
                <div
                  style={{
                    ...
                    fontStyle: "italic",
                    fontSize: 34,
                    clipPath: `inset(0 ${(1 - sigProgress) * 100}% 0 0)`,
                  }}
                >
                  — Yours, The Hive
                </div>
...
const Reveal: React.FC<{ frame: number; at: number; fps: number; children: React.ReactNode }> = ({
  frame,
  at,
  fps,
  children,
...
const Ticker: React.FC<{
  ...
  max: number;
  pad2?: boolean;
}> = ({ frame, startAt, stepFrames, max, pad2 }) => {
