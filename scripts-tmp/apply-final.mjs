...
for (const url of cdnUrls) {
  ...
  const seg = decodeURIComponent(url.split("/").pop().split("?")[0]).replace(
    /[^A-Za-z0-9._-]+/g,
    "_",
  );
  let res, buf, ct;
  try {
    ...
    failed++;
    console.warn("ERR", e.message, url);
    continue;
  }
...
writeFileSync(
  "scripts-tmp/final-apply-manifest.rewritten.json",
...
for (const r of rewritten) {
  ...
  for (const g of r.gallery_images || [])
    if (g && g !== r.primary_image && !imgs.includes(g)) imgs.push(g);
  if (imgs.length === 0) {
    skippedRows++;
    continue;
  }
  ...
  if (error) {
    failedRows++;
    console.warn("DB-FAIL", r.rms_id, error.message);
    continue;
  }
  updated++;
...
writeFileSync(
  "scripts-tmp/apply-result-summary.json",
  JSON.stringify(
    {
      mirrored,
      failedMirror: failed,
      urlsRewritten: rewrittenRefs,
      rowsUpdated: updated,
