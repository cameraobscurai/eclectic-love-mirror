...
console.log(summary);
writeFileSync(
  "scripts/audit/live-vs-catalog-subcat-diff.json",
  JSON.stringify({ summary, mismatches, missing: rows.filter((r) => !r.ourSlug) }, null, 2),
);
...
for (const m of mismatches.slice(0, 25)) {
  console.log(
    `  · ${m.title.padEnd(45)} live=${m.liveCat}/${m.liveSubs.join("|")}  ours=${m.ourCat}/${m.ourSub || "(none)"}`,
  );
}
