...
for (const cat of ["lounge-seating", "lighting"]) {
  const res: any = await fc.scrape(`https://www.eclectichive.com/${cat}`, {
    formats: ["links", "html"],
  ...
  const hrefs = [...html.matchAll(new RegExp(`href="(/${cat}/[^"]+)"`, "g"))]
    .slice(0, 5)
    .map((m) => m[1]);
  ...
  const uniq = [...new Set(allHrefs)].filter(
    (h) => h.split("/").length === 3 && !h.startsWith("/config"),
  );
