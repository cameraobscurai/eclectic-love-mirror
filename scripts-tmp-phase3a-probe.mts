...
const productSchema = {
  ...
  properties: {
    ...
    dimensions: {
      type: "string",
      description: 'Dimensions text exactly as shown (e.g. "32W x 30D x 36H")',
    },
    stocked_quantity: {
      type: "string",
      description: "Stocked / inventory / qty available text if shown, else empty",
    },
    ...
    image_urls: {
      type: "array",
      items: { type: "string" },
      description: "All product gallery image URLs in display order",
    },
...
const result = await fc.scrape(url, {
  formats: [
    ...
    "links",
    {
      ...
      prompt: "Extract product detail page fields. Preserve original strings; do not normalize.",
    },
  ],
