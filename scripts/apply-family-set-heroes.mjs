...
for (const u of unmatched.sort(
  (a, b) => (a.family || "").localeCompare(b.family || "") || a.title.localeCompare(b.title),
)) {
  ...
  JSON.stringify(
    {
      ...
      matches: matches.map((m) => ({
        ...
        category: m.category,
        family: m.family,
        currentHero: m.images?.[0] || null,
        setFile: m.set.filename,
        sqUrl: m.set.imageUrl,
      })),
...
fs.writeFileSync(
  "/mnt/documents/tableware_reshoot_queue.csv",
...
for (const [fam, info] of uniqueSets) {
  try {
    ...
    const { error: upErr } = await supa.storage
      .from(BUCKET)
      .upload(info.targetPath, buf, { contentType: ct, upsert: false, cacheControl: "31536000" });
