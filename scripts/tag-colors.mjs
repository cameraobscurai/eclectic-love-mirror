...
if (!AI_KEY) {
  console.error("LOVABLE_API_KEY required");
  process.exit(1);
}
...
async function pixelExtract(imageUrl) {
  ...
  for (let y = EDGE; y < H - EDGE; y++) {
    for (let x = EDGE; x < W - EDGE; x++) {
      ...
      let isBg = false;
      for (const cl of cornerLabs) {
        if (labDeltaE(lab, cl) < 8) {
          isBg = true;
          break;
        }
      }
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        ...
        hex_secondary: {
          type: "string",
          description: "Optional secondary color hex for patterned/multi items",
        },
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a textile and furniture stylist tagging items for a tonal merchandising grid. Identify the MATERIAL color of the primary object — upholstery, wood, ceramic, metal — that a designer would key off when styling. Ignore: background, props, shadows, reflections, packaging. If the item is patterned or multi-color and no single tone covers >50%, set family=multi and return both hex and hex_secondary.",
      },
      {
        role: "user",
        content: [
...
async function processOne(row) {
  ...
  let pixel = null,
    ...
    error = null;
  try {
    pixel = await pixelExtract(heroUrl);
  } catch (e) {
    error = `pixel: ${e.message}`;
  }
  try {
    ai = await aiVision(heroUrl, row.title);
  } catch (e) {
    error = (error ? error + "; " : "") + `ai: ${e.message}`;
  }
...
console.log(
  `[tag-colors] eligible: ${eligible.length} / ${rows.length}  apply=${APPLY}  retag=${RETAG}`,
);
    ...
    try {
      ...
      const tag = r.skip
        ...
        : r.error
          ...
          : `${r.write.color_family} L=${r.write.color_lightness} ΔE=${r.deltaE?.toFixed(1) ?? "-"}${r.write.color_needs_review ? " ⚠" : ""}`;
      console.log(
        `[${(i + 1).toString().padStart(4, " ")}/${eligible.length}] ${row.title.slice(0, 50).padEnd(50, " ")} ${tag}`,
      );
    } catch (e) {
...
for (const r of results) {
  ...
  if (error) {
    console.error(`update ${r.rms_id}: ${error.message}`);
    continue;
  }
  written++;
}
console.log(
  `[apply] wrote ${written}/${manifest.successful} rows. Now run: bun scripts/bake-catalog.mjs`,
);
