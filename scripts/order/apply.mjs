...
if (!parent) {
  console.error("usage: apply.mjs <parent> [--apply]");
  process.exit(1);
}

const proposal = JSON.parse(
  readFileSync(resolve(root, `scripts-tmp/order/${parent}.proposal.json`), "utf8"),
);
const slice = JSON.parse(
  readFileSync(resolve(root, `scripts-tmp/order/${parent}.slice.json`), "utf8"),
);
...
for (const arr of Object.values(proposal.order))
  for (const id of arr) {
    if (proposalIds.has(id)) {
      console.error(`DUPLICATE in proposal: ${id}`);
      process.exit(2);
    }
    proposalIds.add(id);
  }
...
for (const u of updates) {
  const { error } = await sb
    ...
    .update({ editorial_order: u.order })
    .eq("rms_id", u.rms_id);
  if (error) {
    console.error(`FAIL rms_id=${u.rms_id}:`, error.message);
    process.exit(3);
  }
  ok++;
}
