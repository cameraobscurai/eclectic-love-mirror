...
const parseWidth = (d) => {
  ...
  for (const p of FEET) {
    const m = d.match(p);
    if (m) return sane(Number(m[1]) * 12);
  }
  for (const p of PATTERNS) {
    const m = d.match(p);
    if (m) return sane(Number(m[1]));
  }
  return null;
};
...
for (const k of Object.keys(bySub).sort()) {
  ...
  console.log(
    `  ${k.padEnd(22)} n=${String(a.length).padStart(3)}  median=${a[Math.floor(a.length / 2)]}`,
  );
}
console.log("");
