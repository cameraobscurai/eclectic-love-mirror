...
empty frame. The tile sizes the *file*, not the subject, so those items render
tiny next to neighbours of the same real-world size. No amount of layout math
fixes that; the file has to be normalized. This is the ingest-time fix — the
browser stays dumb.
...
def normalize(im):
    ...
    if max(fill_w, fill_h) >= FILL_TARGET:
...
def main():
    ...
    pairs, report, skipped = [], [], []
    for slug in wanted:
        p = products.get(slug)
        if not p:
            skipped.append((slug, "not in catalog"))
            continue
        url = cover_url(p)
        bucket, path = parse_storage(url or "")
        if not bucket:
            skipped.append((slug, "cover is not a storage object"))
            continue

        try:
            original = Image.open(io.BytesIO(requests.get(url, timeout=90).content)).convert("RGBA")
        except Exception as exc:  # noqa: BLE001 - reported, not swallowed
            skipped.append((slug, f"fetch failed: {exc}"))
            continue

        out, fill = normalize(original)
        if out is None:
            skipped.append((slug, f"already tight {fill[0]:.2f}x{fill[1]:.2f}" if fill else "empty"))
            continue

        print(f"  {slug}: fill {fill[0]:.2f} x {fill[1]:.2f} -> {1 - 2 * PAD:.2f}")
    ...
    if pairs:
        cell = 260
        cols = 4  # two before/after pairs per sheet row
        rows_n = -(-len(pairs) // (cols // 2))
        sheet = Image.new("RGB", (cell * cols, cell * rows_n), (255, 255, 255))
        for i, (_, before, after) in enumerate(pairs):
            ...
            for j, im in enumerate((before, after)):
                ...
                sheet.paste(
                    thumb,
                    ((col * 2 + j) * cell + (cell - thumb.width) // 2,
                     row * cell + (cell - thumb.height) // 2),
                )
        sheet.save(OUT / "contact-sheet.png")
        print(f"\ncontact sheet: {OUT / 'contact-sheet.png'} (pairs: before | after)")

    for slug, why in skipped:
        print(f"  skip {slug}: {why}")

    (OUT / "report.json").write_text(json.dumps(report, indent=2))
    print(f"\n{len(pairs)} to normalize, {len(skipped)} skipped")
    if not args.apply:
