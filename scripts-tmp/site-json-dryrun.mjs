...
while (true) {
  const { data, error } = await sb
    ...
    .select("rms_id,title,category,images,slug")
    .neq("status", "draft")
    .range(from, from + 999);
...
for (const p of all) {
  ...
  const scored = pool
    .map((s) => {
      let score = 0;
      for (const t of ptok) if (s.tokens.has(t)) score++;
      return { s, score };
    })
    .filter((x) => x.score >= 2)
  ...
  const rec = {
    ...
    our_title: p.title,
    cat: p.category,
    site_title: top.s.title,
    urlId: top.s.urlId,
    hero: top.s.hero,
    gallery_count: top.s.urls.length,
    urls: top.s.urls,
    score: top.score,
...
const summary = [
  ...
  ...result.blanks_HIGH
    ...
    .map(
      (r) =>
        `  [${r.score}] ${r.cat.padEnd(16)} rms${r.rms} "${r.our_title}" ← "${r.site_title}" (${r.gallery_count} imgs)`,
    ),
  "",
  "## NONE (no match):",
