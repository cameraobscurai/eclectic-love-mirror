...
`/admin/new-product`'s button says **"Publish to live catalog"** but only flips a database flag. The public site reads a baked snapshot that is only rebuilt by a *different* button on `/admin/photos`. Adrienne publishes, sees nothing on the live site, concludes it's broken.
...
One bespoke behavior must survive the Form swap: the drawer advances its baseline only for *untouched* fields, so a mid-edit refetch can't stomp what she's typing. That gets ported explicitly, not assumed.
