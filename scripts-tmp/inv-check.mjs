...
while (true) {
  const { data, error } = await sb
    ...
    .select(
      "rms_id,title,slug,category,quantity,quantity_label,dimensions_raw,images,description,status",
    )
    .neq("status", "draft")
    .range(from, from + 999)
    .order("category")
...
let imgReport = null;
try {
  imgReport = JSON.parse(fs.readFileSync("scripts-tmp/per-product-image-report.json", "utf8"));
} catch {}
...
for (const r of all) {
  ...
  if (liveByTitle.has(k)) {
    liveMatches++;
    continue;
  }
  const tokens = k.split(" ");
  for (let i = tokens.length - 1; i >= 2; i--) {
    if (liveByTitle.has(tokens.slice(0, i).join(" "))) {
      liveMatches++;
      break;
    }
  }
