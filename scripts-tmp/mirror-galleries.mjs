...
const plans = {
  ...
  "BRUSH-CREEK-LOVE-THIS-DAY": (f) =>
    f.path.startsWith("Favorites/") || f.path.startsWith("Favorites"),
...
function bishops(files) {
  ...
  const tent = files
    .filter((f) => f.path.startsWith("Details_-_Tent/"))
    .sort((a, b) => a.path.localeCompare(b.path))
    .slice(0, 10);
  const fri = files
    .filter((f) => f.path.startsWith("Friday_-_Details/"))
    .sort((a, b) => a.path.localeCompare(b.path))
    .slice(0, 5);
    ...
    while (queue.length) {
      ...
      try {
        ...
        const { error } = await sb.storage
          ...
          .upload(`${dstKey}/${name}`, buf, { contentType: ct, upsert: true });
        if (error) throw error;
        ok++;
        if (ok % 10 === 0) console.log(`  ${ok}/${picked.length}`);
      } catch (e) {
        fail++;
        errs.push(f.path + ": " + e.message);
      }
    }
  }
