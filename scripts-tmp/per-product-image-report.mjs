...
function existsInStorage(objectPath) {
  return objectPath ? storageSet.has(objectPath) : false;
}
...
const rows = products.map((p) => {
  ...
  // Decide proposal
  let proposed_hero_url = null,
    proposed_object_path = null,
...
const breakUnfixable = rows.filter(
  (r) => r.status_class === "SHARED_BREAK" && r.repair_kind === "none",
).length;
const imagelessUnfixable = rows.filter(
  (r) => r.status_class === "IMAGELESS" && r.repair_kind === "none",
).length;
...
for (const [k, n] of Object.entries(counts.status_class).sort((a, b) => b[1] - a[1]))
  lines.push(`  ${k.padEnd(14)} ${n}`);
lines.push("");
lines.push("REPAIR_KIND (proposed action per product)");
for (const [k, n] of Object.entries(counts.repair_kind).sort((a, b) => b[1] - a[1]))
  lines.push(`  ${k.padEnd(28)} ${n}`);
lines.push("");
lines.push("PROPOSED_SOURCE");
for (const [k, n] of Object.entries(counts.proposed_source).sort((a, b) => b[1] - a[1]))
  lines.push(`  ${k.padEnd(28)} ${n}`);
lines.push("");
lines.push("FLAGS");
