...
const folderCounts = [...folderStats.values()].sort((a, b) => b.image_count - a.image_count);

writeFileSync("scripts-tmp/storage-filetree-manifest.json", JSON.stringify(manifest, null, 2));
writeFileSync("scripts-tmp/storage-folder-counts.json", JSON.stringify(folderCounts, null, 2));

// txt versions
const ftLines = [
  "# Storage filetree manifest",
  "",
...
for (const f of folderCounts) {
  fcLines.push(
    `## ${f.folder}  →  ${f.mapped_category || "(unmapped)"}  [${f.category_certainty}]`,
  );
...
function findMatchesForFile(file) {
  ...
  for (const r of pool) {
    ...
    if (stemKey && stemKey === r.norm_title) {
      method = "exact_filename_eq_title";
      confidence = 1.0;
    }
    ...
    // 3. token overlap
      ...
      if (j >= 0.66) {
        method = "token_overlap_strong";
        confidence = j;
      } else if (j >= 0.45) {
        method = "token_overlap_medium";
        confidence = j;
      }
    }
    ...
    if (isStrict) {
      const sharedNonGeneric = [...stemTokens].some(
        (t) => r.title_tokens.has(t) && !SMALLWARE_TOKENS.has(t),
      );
...
for (const m of fileMatches) {
  ...
  if (!byRmsId.has(id))
    byRmsId.set(id, {
      rms_id: id,
      slug: m.top_match.slug,
      title: m.top_match.title,
      category: m.top_match.category,
      files: [],
    });
...
const productGroups = [...byRmsId.values()].map((g) => {
  ...
  const ranked = [...g.files].sort((a, b) => {
    const ca = a.top_match.confidence,
      cb = b.top_match.confidence;
...
const APPLY_SAFE = fileMatches.filter(
  (m) => m.bucket === "APPLY_SAFE" && !m.top_match.already_has_image,
);
const REVIEW = fileMatches.filter((m) => m.bucket === "REVIEW");
const UNMATCHED = fileMatches.filter((m) => m.bucket === "UNMATCHED");
const POTENTIAL_DUPLICATES = fileMatches.filter(
  (m) => m.bucket === "APPLY_SAFE" && m.top_match.already_has_image,
);
...
const CATEGORY_MISMATCHES = fileMatches.filter(
  (m) =>
    m.top_match &&
    m.mapped_category &&
    m.top_match.category &&
...
for (const r of STILL_IMAGELESS)
  missingByCat.set(r.category, (missingByCat.get(r.category) || 0) + 1);
...
writeFileSync(
  ...
  JSON.stringify(STILL_IMAGELESS, null, 2),
);
writeFileSync(
  "scripts-tmp/potential-duplicates-hard-files.json",
  JSON.stringify(POTENTIAL_DUPLICATES, null, 2),
);
writeFileSync(
  "scripts-tmp/category-mismatches-hard-files.json",
  JSON.stringify(CATEGORY_MISMATCHES, null, 2),
);
