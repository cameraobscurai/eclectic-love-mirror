...
const SIZE_RX =
  /^(?:\d{1,3}(?:["'']|in|inch|ft|feet|cm)?|x?xs|xs|sm|small|med|medium|lg|large|xl|xxl)$/;
const STOP = new Set([
  "the",
  "a",
  "an",
...
for (const slug of liveSlugs) {
  ...
  if (hits.length === 1) row = hits[0];
    ...
    ambiguous.push({
      ...
      candidates: hits.map((h) => ({ rms_id: h.rms_id, category: h.category, db_title: h.title })),
    });
    continue;
  } else {
    ...
    if (fz) {
      ...
      unmatched.push({ slug, title: p.title, categories: p.categories });
      continue;
    }
  }
  ...
  const seen = new Set();
  const proposed = [];
  for (const u of liveMirrored)
    if (!seen.has(u)) {
      seen.add(u);
      proposed.push(u);
    }
  for (const u of ownerKept)
    if (!seen.has(u)) {
      seen.add(u);
      proposed.push(u);
    }
  ...
  matched.push(rec);
  if (matchKind === "fuzzy")
    fuzzyMatched.push({ rms_id: row.rms_id, db_title: row.title, live_title: p.title });
}
...
const lines = [
  ...
  ...fuzzyMatched
    .slice(0, 15)
    .map((f) => `  • live="${f.live_title}"  →  db="${f.db_title}"  rms=${f.rms_id}`),
  "",
  "Sample unmatched (first 15):",
  ...unmatched
    .slice(0, 15)
    .map((u) => `  • ${u.title}  [${u.categories.join(",")}]  slug=${u.slug}`),
  "",
  "Outputs:",
