...
async function listAAM() {
  const r = await fetch(
    `${GW}/files?q=${encodeURIComponent(`'1RiRyTB289dR2I_UPHbNZmJURWRgXGzsx' in parents and trashed=false`)}&fields=files(id,name)&pageSize=200`,
    { headers: H },
  );
...
for (const f of files) {
  try {
    ...
    const { error } = await sb.storage
      ...
      .upload(`BIRCH-DESIGN/${name}`, buf, { contentType: "image/jpeg", upsert: true });
