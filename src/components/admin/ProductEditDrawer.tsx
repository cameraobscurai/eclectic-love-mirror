...
/* ═════════════ DEMO HARNESS — delete on port ═════════════ */

const Sk = ({ children }) => (
  <svg viewBox="0 0 100 100" fill="none" stroke={T.charcoal} strokeWidth="1.4" strokeLinecap="round" style={{ width: "100%", opacity: 0.85 }}>{children}</svg>
);
const SKETCHES = {
  p1: <Sk><path d="M30 12 L30 52 M70 12 L70 52 M30 18 Q50 12 70 18 M30 30 Q50 24 70 30 M30 44 Q50 40 70 44 M26 52 L74 52 M28 52 L24 92 M72 52 L76 92 M33 52 L36 92 M67 52 L64 92 M30 70 L68 72" /></Sk>,
  p2: <Sk><ellipse cx="50" cy="30" rx="42" ry="10" /><path d="M8 30 L8 36 Q50 48 92 36 L92 30 M50 40 L50 78 M32 92 Q50 82 68 92 M50 78 L32 92 M50 78 L68 92" /></Sk>,
  p3: <Sk><path d="M28 20 L44 20 L44 26 M28 20 L28 26 M26 26 L46 26 L44 70 L28 70 Z M36 12 Q42 12 42 20 M36 26 L36 70 M26 70 L46 70 M58 34 L74 34 M56 34 L76 34 L74 78 L58 78 Z M66 26 Q72 26 72 34 M66 34 L66 78" /></Sk>,
};

const MOCK_CATEGORIES = ["seating", "tables", "lighting", "tableware", "textiles", "large-decor"];

const MOCK_PRICE_STATS = {
  seating: { min: 12, median: 18, max: 45 },
  tables: { min: 85, median: 145, max: 320 },
  lighting: { min: 24, median: 65, max: 180 },
  tableware: { min: 2, median: 6, max: 22 },
};

/* Statuses use the REAL item_status enum. p3 demonstrates the draft-prep flow. */
const MOCK_PRODUCTS = [
  {
    id: "p1", rms_id: "EH-2041", title: "Gerwyn Grey Dining Chair", slug: "gerwyn-grey-dining-chair",
    category: "seating", status: "available", public_ready: true, quantity: 24, quantity_label: null,
    price: 18, description: "A quietly sculptural oak frame in a smoked grey finish — the workhorse of long-table dinners.",
    dimensions_raw: '19"W × 21"D × 33"H', materials: "Oak, linen", origin: "Round Top Antiques Fair, TX",
    editorial_order: 4, hidden_note: "", images: ["x"], meta_title: "", meta_description: "",
    og_image: "", manual_injection: false,
  },
  {
    id: "p2", rms_id: "EH-1187", title: "Kohl Round Black Dining Table", slug: "kohl-round-black-dining-table",
    category: "tables", status: "available", public_ready: true, quantity: 6, quantity_label: null,
    price: 145, description: "Matte black round dining table seating eight. Pairs with almost everything in the archive.",
    dimensions_raw: '60"Ø × 30"H', materials: "Painted oak", origin: "",
    editorial_order: 1, hidden_note: "Two tops have minor edge wear — face away from camera.", images: ["x"],
    meta_title: "", meta_description: "", og_image: "", manual_injection: false,
  },
  {
    id: "p3", rms_id: "EH-3302", title: "Layla Lantern Pair", slug: "layla-lantern-pair",
    category: "lighting", status: "draft", public_ready: false, quantity: 2, quantity_label: "pairs",
    price: "", description: "", dimensions_raw: "", materials: "Brass, glass", origin: "",
    editorial_order: null, hidden_note: "Arriving from supplier — ETA Aug 4.", images: [],
    meta_title: "", meta_description: "", og_image: "", manual_injection: false,
  },
];

/* Audit seed in the REAL admin_audit_log shape — whole-row snapshots.
   Proves flattenAuditRows() does the diffing the route no longer has to. */
const hoursAgo = (h) => new Date(Date.now() - h * 3600000).toISOString();
const MOCK_AUDIT_SEED = [
  {
    id: "aud-1", at: hoursAgo(2), actor_id: "adrienne", action: "update",
    before: { title: "Gerwyn Grey Dining Chair", price: 16, public_ready: true },
    after: { title: "Gerwyn Grey Dining Chair", price: 18, public_ready: true },
    metadata: null,
  },
  {
    id: "aud-2", at: hoursAgo(26), actor_id: "adrienne", action: "update",
    before: { public_ready: false, quantity: 24 },
    after: { public_ready: true, quantity: 24 },
    metadata: null,
  },
];
...
export default function App() {
  ...
  const [audit, setAudit] = useState(MOCK_AUDIT_SEED);
  ...
  const handleSave = async (patch) => {
    ...
    const prev = products.find((p) => p.id === openId);
    setProducts((ps) => ps.map((p) => (p.id === openId ? { ...p, ...patch } : p)));
    /* Append a snapshot-shaped audit row, exactly like the DB trigger would */
    setAudit((a) => [{
      id: `aud-${Date.now()}`, at: new Date().toISOString(), actor_id: "you", action: "update",
      before: { ...prev }, after: { ...prev, ...patch }, metadata: null,
    }, ...a].slice(0, 5));
  };

  return (
    <div className="min-h-screen" style={{ background: T.paper, fontFamily: T.sans }}>
      <div style={{ borderBottom: T.hairline, background: T.white, padding: "18px 28px" }}
        className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <p style={micro(9, T.trackUltra, "rgba(26,26,26,0.4)")}>Eclectic Hive · Hive Signature Collection</p>
          <h1 style={{ fontFamily: T.serif, fontSize: 22, fontWeight: 400, color: T.charcoal, marginTop: 4 }}>Inventory</h1>
        </div>
        <div className="flex items-center gap-2">
          <span style={micro(9, T.trackLabel, "rgba(26,26,26,0.45)")}>Viewing as</span>
          {["staff", "admin"].map((r) => (
            <button key={r} onClick={() => setRole(r)}
              style={{
                ...micro(9, T.trackLabel, role === r ? T.paper : "rgba(26,26,26,0.55)"),
                background: role === r ? T.charcoal : "none",
                border: T.hairline, padding: "7px 12px", cursor: "pointer",
              }}>{r === "staff" ? "Adrienne (staff)" : "Admin"}</button>
          ))}
        </div>
      </div>

      <div style={{ maxWidth: 860, margin: "0 auto", padding: "34px 28px" }}>
        <p style={{ fontSize: 12, color: "rgba(26,26,26,0.5)", marginBottom: 18, lineHeight: 1.6 }}>
          Statuses now use the real <code style={{ background: "rgba(26,26,26,0.05)", padding: "1px 5px" }}>item_status</code> enum,
          and the Recent Changes feed ingests raw audit-log snapshots — edit the Gerwyn chair,
          save, and watch the diff appear, then Undo just that field.
        </p>
        <div style={{ border: T.hairline, background: T.white }}>
          {products.map((p, i) => {
            const r = computeReadiness(
              Object.fromEntries(Object.keys(FIELD_META).map((k) => [k, p[k] ?? ""])), p
            );
            return (
              <button key={p.id} onClick={() => setOpenId(p.id)}
                className="w-full text-left flex items-center justify-between gap-4"
                style={{ padding: "16px 20px", background: "none", cursor: "pointer", border: "none", borderTop: i === 0 ? "none" : T.hairlineSoft }}
                onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(26,26,26,0.025)")}
                onMouseLeave={(e) => (e.currentTarget.style.background = "none")}>
                <div className="min-w-0">
                  <p style={{ fontFamily: T.serif, fontSize: 16, color: T.charcoal, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{p.title}</p>
                  <p style={{ ...micro(9, T.trackLabel, "rgba(26,26,26,0.42)"), marginTop: 4 }}>
                    {p.category} · № {p.rms_id} · {STATUS_OPTIONS.find((s) => s.value === p.status)?.label ?? p.status}
                  </p>
                </div>
                <div className="flex items-center gap-4 flex-shrink-0">
                  {!r.ready && <span style={micro(8.5, T.trackLabel, "rgba(26,26,26,0.4)")}>{r.checks.filter((c) => c.pass).length}/{r.checks.length} ready</span>}
                  <span style={micro(9, T.trackLabel, p.public_ready ? "rgba(26,26,26,0.5)" : "rgba(140,47,34,0.75)")}>
                    {p.public_ready ? "Live" : "Hidden"}
                  </span>
                </div>
              </button>
            );
      ...
      {note && (
        <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", background: T.charcoal, color: T.paper, fontSize: 12, padding: "10px 18px", fontFamily: T.sans, zIndex: 60 }}>{note}</div>
      )}
      ...
      {open && (
        <ProductEditDrawer
          ...
          categoryPriceStats={MOCK_PRICE_STATS}
          role={role}
          recentChanges={openId === "p1" ? audit : []}
          liveUrl={`https://eclectichive.com/collection/${open.slug}`}
          sketch={SKETCHES[open.id]}
          onSave={handleSave}
          onOpenPhotos={() => { setNote("On port: opens your existing ImageOrderEditor"); setTimeout(() => setNote(null), 2200); }}
