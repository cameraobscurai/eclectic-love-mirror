...
for (const p of catalog.products) {
  for (const img of p.images || []) {
    ...
    if (usedPaths.has(target) && usedPaths.get(target) !== url) {
      ...
      let i = 2;
      while (
        usedPaths.has(`${dir}/${stem}_${i}${ext}`) &&
        usedPaths.get(`${dir}/${stem}_${i}${ext}`) !== url
      )
        i++;
  ...
  while (queue.length) {
    ...
    try {
      ...
      const { error } = await sb.storage
        .from(BUCKET)
        .upload(job.targetPath, buf, { contentType: ct, upsert: false });
      if (error) {
        if (/exists|duplicate/i.test(error.message)) {
          skip++;
          results.push({ ...job, status: "skip-exists" });
        } else throw error;
      } else {
        ok++;
        results.push({ ...job, status: "ok", bytes: buf.length });
      }
    } catch (e) {
      fail++;
      results.push({ ...job, status: "fail", error: String(e.message || e) });
    } finally {
      done++;
      if (done % 50 === 0)
        console.log(`  ${done}/${plan.length}  ok=${ok} skip=${skip} fail=${fail}`);
    }
  }
}
...
fs.writeFileSync(
  "/tmp/collection-snapshot-result.json",
