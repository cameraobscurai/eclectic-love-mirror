...
const siteImages = monroeSite
  ? monroeSite.cdn_image_urls.map((u) => `${u.split("?")[0]}?format=2500w`)
  : [];
...
const monroeRows = MONROE.rms_rows.map((row) => {
  ...
  return {
    ...
    confidence:
      storage && sitePerSize.length
        ? "high (size match in both sources)"
        : storage
          ? "medium (storage size match only)"
          : sitePerSize.length
            ? "medium (site filename size match only)"
            : "low (fallback to generic site image)",
    notes:
      'CSV labels filenames as "Astaire" but RMS title is "Monroe — Black Slat Tambour". Storage filenames already use correct "MONROE" naming. Visual review required: confirm size labels (5\'/8\'/10\'/16\'/15\') match physical bar.',
  };
});

// --- Two conflicts (Iraja, Gideon) ---
const conflictRows = CONFLICTS.map((c) => {
  const siteMatches = SITE.filter((p) =>
    p.title.toLowerCase().includes(c.product_name.split(" ")[0].toLowerCase()),
  );
  return {
    ...
    recommended_primary_image: siteMatches[0]?.cdn_image_urls[0]
      ...
      : c.storage_candidate,
    recommended_gallery_images: (siteMatches[0]?.cdn_image_urls || [])
      .slice(1)
      .map((u) => `${u.split("?")[0]}?format=2500w`),
    confidence:
      c.product_id === "2864"
...
console.log(
  `Site images for Monroe: ${siteImages.length}, storage MONROE files: ${MONROE.storage_files.filter((p) => /MONROE/.test(p)).length}`,
);
