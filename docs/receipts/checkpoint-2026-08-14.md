...
| Catalog merge           | Admin authority beats family promotion (`family-cover.ts:113`); tombstones filter products _and_ variants; `upscaled_cover_url` has zero references in app code and zero non-null rows; `family_option_name` + `images_version` flow through the overlay; `?v=` derives from `updated_at`                                                                                                |
...
1. **Lighting scale intent.** Memory says lighting is exempt from true physical
   ...
   arguably _should_ be floor-standing, so this needs a human call before a code
