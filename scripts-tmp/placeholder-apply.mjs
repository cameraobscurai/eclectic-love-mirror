...
const fname = (u, i) => {
  try {
    const p = new URL(u).pathname.split("/").pop() || `image-${i}`;
...
for (const t of targets) {
  ...
  const { data: row, error: rErr } = await sb
    ...
    .maybeSingle();
  if (rErr || !row) {
    results.push({ rms_id: t.rms_id, status: "fail", error: "row not found" });
    continue;
  }
...
writeFileSync(
  "scripts-tmp/placeholder-apply-result.json",
  JSON.stringify({ generated_at: new Date().toISOString(), ok, fail, results }, null, 2),
);
