...
for (const g of groups.values()) {
  ...
  if (pick) {
    matched.push({
      ...
      runnerUps: candidates.length
        ? candidates.slice(1, 4).map((c) => ({ title: c.title, category: c.category }))
        : [],
    });
  } else if (candidates.length > 1) {
    ambiguous.push({
      ...g,
      candidates: candidates.map((c) => ({ id: c.id, title: c.title, category: c.category })),
    });
  } else {
    orphans.push(g);
  }
