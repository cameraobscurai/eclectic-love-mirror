...
console.log(
  "Falling back to: snapshot first primaryImage URL for each products[].categorySlug, plus overrides keyed by their slug.",
);
...
for (const p of cat.products) {
  ...
  if (p.primaryImage?.url)
    firstByCat.set(p.categorySlug, { url: p.primaryImage.url, slug: p.slug, title: p.title });
}
