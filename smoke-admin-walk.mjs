...
log('a) /admin/products loads with rows', rowCount > 0 && noMatchText === 0, `rowCount=${rowCount}, url=${page.url()}`);
...
for (const label of colOptions.slice(0, 6)) { // cap to keep runtime reasonable, but note if truncated
...
log('c) product edit drawer opens & renders fields', urlHasId && drawerInputs > 5, `url=${page.url()}, formControls=${drawerInputs}`);
...
log('d) Publish bar renders on /admin/photos', publishText > 0, `publishMatches=${publishText}, url=${page.url()}`);
...
const chipText = await page.locator('button:has-text("Confirm")').first().textContent().catch(() => null);
log('e) /admin/taxonomy loads with chip counts', chipButtons > 0, `chipButtonsWithDigits=${chipButtons}, sampleChip="${chipText}"`);
...
log('f) inquiries inbox opens (redirects to /admin/insights)', page.url().includes('/admin/insights'), `finalUrl=${page.url()}`);
