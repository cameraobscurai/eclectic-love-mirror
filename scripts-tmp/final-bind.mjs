...
function ownerTitleNorm(t) {
  return applyAliases(normKey(t));
}
...
function pickPrimaryAndGallery(files) {
  ...
  if (setFile) {
    primary = setFile;
    rest = sorted.filter((f) => f !== setFile);
  } else if (sorted.length) {
    primary = sorted[0];
    rest = sorted.slice(1);
  } else {
    primary = renders[0];
    rest = renders.slice(1);
  }
...
for (const r of rmsRows) {
  ...
  if (!primary && titleTokens.length && folderFiles.length) {
    ...
    if (cand.length === 0)
      cand = folderFiles.filter((f) => f.norm.startsWith(stem1 + " ") || f.norm === stem1);
    if (cand.length) {
      ...
      if (pieceHit) {
        ...
        if (setF && setF !== pieceHit) {
          ...
          gallery = [
            setF.public_url,
            ...cand.filter((f) => f !== pieceHit && f !== setF).map((f) => f.public_url),
          ];
        } else {
