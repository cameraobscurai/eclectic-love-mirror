...
const UNFRIENDLY_ERROR =
  /\b(undefined|null|NaN|\[object Object\]|TypeError|violates row-level security|PGRST\d+|22P02|23505|Unexpected token|Failed to fetch)\b/;
...
async function visibleErrorText(page: Page): Promise<string[]> {
  return page.evaluate(() => {
    ...
    const nodes = document.querySelectorAll(
      '[role="alert"], .text-red-600, [data-sonner-toast][data-type="error"]',
    );
    nodes.forEach((n) => {
...
async function deleteArtifact(page: Page, id: string) {
  await page.goto(`http://localhost:8080/admin/products?id=${id}`, {
...
test.describe("Inventory add/edit — staff walkthrough", () => {
  ...
  test("create → land in editor → edit → persists → findable in search", async ({
    page,
    context,
  }) => {
    const h = watch(page);
    ...
    expect(page.url(), "staff user was redirected away from /admin/new-product").toContain(
      "/admin/new-product",
    );
    ...
    await categorySelect.selectOption({ label: "Cocktail & Bar" }).catch(async () => {
      ...
      const values = await categorySelect
        .locator("option")
    ...
    expect(
      subOptionCount,
      "no subcategory options offered for the selected category",
    ).toBeGreaterThan(1);
    ...
    expect(
      (await drawerCollection.inputValue()).trim(),
      "collection lost on the way into the editor",
    ).not.toBe("");

    const drawerCategory = page.locator("#f-category_slug");
    await expect(drawerCategory).toBeVisible();
    expect(
      (await drawerCategory.inputValue()).trim(),
      "category lost on the way into the editor",
    ).not.toBe("");
    expect(
    ...
    await page.goto(`http://localhost:8080/admin/products?id=${productId}`, {
      waitUntil: "domcontentloaded",
    });
    ...
    expect(
      unfriendly,
      `raw/unhelpful error copy shown to the user:\n${unfriendly.join("\n")}`,
    ).toEqual([]);
    expect(
      h.consoleErrors,
  ...
  test("validation is human-readable, not raw database errors", async ({ page, context }) => {
    ...
    await page.getByPlaceholder("e.g. Travertine Side Table").fill("");
    await expect(
      page.getByText(/add a title and choose where it lives to enable photo uploads/i),
    ).toBeVisible();
