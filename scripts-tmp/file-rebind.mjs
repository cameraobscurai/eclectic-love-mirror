...
while (true) {
  const { data, error } = await sb.storage
    .from("inventory")
    .list("inventory", { limit: 1000, offset: off });
  ...
  for (const f of data) {
    ...
    while (true) {
      const r = await sb.storage
        .from("inventory")
        .list(`inventory/${f.name}`, { limit: 1000, offset: foff });
...
for (const f of files) {
  ...
  const scored = candidates
    .map((p) => {
      ...
      let s = 0;
      for (const x of fileTokens) if (t.has(x)) s++;
  ...
  proposals.push({
    ...
    cat,
    score: best.score,
    suggest_rms: best.p.rms_id,
    suggest_title: best.p.title,
    current_hero_path: currentHero || null,
    file_already_used_by_other_products: usedByOther.map((x) => ({
      rms: x.rms_id,
      title: x.title,
    })),
  });
...
fs.writeFileSync(
  "/dev-server/scripts-tmp/file-rebind-proposals.json",
  JSON.stringify({ fixes, ties }, null, 2),
);
console.log("proposals:", fixes.length, "ties:", ties.length);
console.log("Sample fixes:");
for (const f of fixes.slice(0, 15))
  console.log(" ", f.file, "→", f.suggest_rms, f.suggest_title, "(score", f.score + ")");
