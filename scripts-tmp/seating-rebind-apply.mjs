...
const fname = (u, i) => {
  try {
    return decodeURIComponent(new URL(u).pathname.split("/").pop() || `image-${i}`);
  } catch {
    return `image-${i}`;
  }
};
...
for (const t of targets) {
  ...
  if (!m) {
    results.push({ rms_id: t.rms_id, title: t.title, status: "no_truth" });
    continue;
  }
  ...
  if (!urls.length) {
    results.push({ rms_id: t.rms_id, title: t.title, status: "no_cdn" });
    continue;
  }
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      if (buf.length < 5000) {
        skipped.push({ url: u, reason: `placeholder bytes=${buf.length}` });
        continue;
      }
      const { error } = await sb.storage
        .from(BUCKET)
        .upload(path, buf, { upsert: true, contentType: ctype(u), cacheControl: "31536000" });
...
writeFileSync(
  "scripts-tmp/seating-rebind-result.json",
  JSON.stringify(
    { generated_at: new Date().toISOString(), ok, fail: results.length - ok, results },
    null,
    2,
  ),
);
