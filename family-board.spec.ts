...
const UNFRIENDLY = /\b(undefined|null|NaN|\[object Object\]|TypeError|violates row-level security|PGRST\d+|22P02|23505)\b/;
...
async function restoreSession(page: Page, context: BrowserContext) {
  ...
  if (key && session) {
    await page.evaluate(([k, s]) => window.localStorage.setItem(k as string, s as string), [key, session]);
  }
}
...
test.describe('Family board — writable variant workflow', () => {
  ...
  test('label, order, pin and reset all persist across a reload', async ({ page, context }) => {
    ...
    await expect(afterReload, 'variant label did not survive a reload').toHaveValue(probe, { timeout: 20_000 });
    ...
    if (await thumbs.count()) {
      ...
      await page.reload({ waitUntil: 'domcontentloaded' });
      await expect(page.getByText('Pinned photo').first(), 'pinned photo did not persist').toBeVisible({
        timeout: 20_000,
      });

      const resetRow = page.locator('li', { has: page.locator('input[aria-label^="Variant name for"]') }).first();
    ...
    if ((await rows.count()) > 1) {
      ...
      await page.reload({ waitUntil: 'domcontentloaded' });
      await expect(page.getByText('Part of a collection')).toBeVisible({ timeout: 20_000 });
      const rowsAfter = page.locator('li', { has: page.locator('input[aria-label^="Variant name for"]') });
      await expect
        .poll(async () => rowsAfter.nth(0).locator('input').first().inputValue(), { timeout: 20_000 })
  ...
  test('a photo that is not this variant\'s own cannot be pinned', async ({ page, context }) => {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      const thumbs = await row.locator('button[title*="photo"] img').evaluateAll((imgs) =>
        imgs.map((img) => (img as HTMLImageElement).src),
      );
      expect(new Set(thumbs).size, 'the same photo is offered twice inside one variant').toBe(thumbs.length);
    }

    const alerts = await page.evaluate(() =>
      Array.from(document.querySelectorAll('[role="alert"], [data-sonner-toast][data-type="error"]')).map(
        (n) => n.textContent ?? '',
      ),
    );
