...
const RULES = {
  seating: { anchor: "bottom", anchorY: 0.9, maxW: 0.86, maxH: 0.8 },
  tables: { anchor: "bottom", anchorY: 0.9, maxW: 0.86, maxH: 0.8 },
  bars: { anchor: "bottom", anchorY: 0.9, maxW: 0.8, maxH: 0.84 },
  lighting: { anchor: "bottom", anchorY: 0.92, maxW: 0.72, maxH: 0.84 },
  chandeliers: { anchor: "top", anchorY: 0.08, maxW: 0.78, maxH: 0.8 },
  candlelight: { anchor: "bottom", anchorY: 0.85, maxW: 0.62, maxH: 0.76 },
  tableware: { anchor: "center", anchorY: 0.5, maxW: 0.7, maxH: 0.7 },
  serveware: { anchor: "center", anchorY: 0.5, maxW: 0.74, maxH: 0.74 },
  "pillows-throws": { anchor: "center", anchorY: 0.5, maxW: 0.76, maxH: 0.76 },
  rugs: { anchor: "center", anchorY: 0.55, maxW: 0.86, maxH: 0.8 },
  "large-decor": { anchor: "bottom", anchorY: 0.9, maxW: 0.82, maxH: 0.84 },
  storage: { anchor: "bottom", anchorY: 0.9, maxW: 0.84, maxH: 0.82 },
  styling: { anchor: "center", anchorY: 0.55, maxW: 0.7, maxH: 0.72 },
...
export async function normalizeCover(buf, categorySlug, { canvas = CANVAS } = {}) {
  ...
  return {
    buf: out,
    note: `${box.width}x${box.height} -> ${w}x${h} @ ${rule.anchor}/${rule.anchorY}`,
  };
}
...
export async function padToSquare(buf, { margin = 0.06 } = {}) {
  ...
  const side = Math.round(
    Math.max(box?.width ?? meta.width, box?.height ?? meta.height) * (1 + margin * 2),
  );
  const src = box
    ? await base
        .extract({ left: box.left, top: box.top, width: box.width, height: box.height })
  ...
  return sharp({ create: { width: side, height: side, channels: 3, background: "#ffffff" } })
    .composite([
      {
        input: src,
        left: Math.round((side - s.width) / 2),
        top: Math.round((side - s.height) / 2),
      },
    ])
    .png()
    .toBuffer();
