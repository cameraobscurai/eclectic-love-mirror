...
export function BohHome({ firstName: firstNameProp }: { firstName?: string }) {
    ...
    if (i >= 0 && !zoom) {
      ...
      setZoom({
        ...
        rect: rect
          ...
          : { left: innerWidth / 2 - 150, top: innerHeight / 2 - 100, width: 300 },
      });
    }
  ...
  const openZoom = useCallback(
    (index: number, rect?: DOMRect) => {
      ...
      navigate({
        search: ((s: { page?: string }) => ({ ...s, page: PAGES[index].slug })) as never,
        replace: false,
      });
    },
    [navigate],
  ...
  const closeZoom = useCallback(() => {
    ...
    setZoom(null);
    navigate({
      search: ((s: { page?: string }) => ({ ...s, page: undefined })) as never,
      replace: false,
    });
  ...
  const newest = snapshots.reduce<string | null>(
    (m, s) => (s.updated_at && (!m || s.updated_at > m) ? s.updated_at : m),
    null,
  );
    ...
    <div style={{ minHeight: "100vh", background: T.bg, color: T.ink, fontFamily: T.sans }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "baseline", gap: 16 }}>
          <span
            style={{ fontFamily: T.serif, fontWeight: 600, fontSize: 17, letterSpacing: "0.22em" }}
          >
            ECLECTIC HIVE
          </span>
          <span style={{ color: T.faint }}>—</span>
          <span style={{ fontSize: 11, letterSpacing: "0.26em", color: "#CFC9BD" }}>
            BACK OF HOUSE
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 22 }}>
          ...
          <button
            onClick={() => window.dispatchEvent(new Event("boh:open-command"))}
            className="boh-ghost-btn"
          >
            ⌘K · SEARCH
          </button>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3 }}>
            <button
              ...
              disabled={sweeping}
              className="boh-ghost-btn"
              aria-label="Refresh page snapshots"
            >
              {sweeping ? "↻ CAPTURING…" : "↻ REFRESH"}
            </button>
            {staleness && (
              <span style={{ fontSize: 9, letterSpacing: "0.14em", color: T.dim }}>
                LAST REFRESHED {staleness}
              </span>
            )}
          </div>
        </div>
      ...
      >
        ...
        >
          <RibbonLink
            ...
            text={ribbon ? `${ribbon.totalItems} ITEMS` : "— ITEMS"}
          />
          <Dot />
          <RibbonLink
            onClick={() => go("/admin/photos?filter=missing")}
            text={ribbon ? `${ribbon.imagedPct}% IMAGED` : "—% IMAGED"}
          />
          <Dot />
          <RibbonLink
          ...
          />
          {ribbon?.lastPublishedAt && (
            <>
              <Dot />
              <span>PUBLISHED {relTime(ribbon.lastPublishedAt)}</span>
            </>
          )}
        </div>
      </div>
      ...
      >
        <div style={{ fontSize: 10, letterSpacing: "0.26em", color: T.dim, marginBottom: 12 }}>
          START HERE
        </div>
        ...
        {TOOL_RAIL.map((t) => {
          ...
          return (
            ...
            >
                ...
                {count != null && (
                  <span
                    style={{
                      fontFamily: T.mono,
                      fontSize: 11,
                      color: accent ? T.attention : T.dim,
              ...
              >
                <span
                  style={{ fontSize: 11, letterSpacing: "0.14em", color: T.muted, lineHeight: 1.4 }}
                >
                  {t.desc.toUpperCase()}
                </span>
                <span style={{ fontSize: 10, letterSpacing: "0.2em", color: T.faint }}>→</span>
      ...
      >
        <button onClick={() => setCatsOpen((v) => !v)} className="boh-strip-toggle">
          <span>
            ...
            HIVE SIGNATURE COLLECTION — {NAV_GROUPS.length} CATEGORY VIEWS
          </span>
          ...
          >
            {NAV_GROUPS.map((g, i) => {
              ...
              return (
                <button
                  key={g.slug}
                  onClick={() => go(`/admin/products?group=${g.slug}`)}
                  className="boh-cat-card"
                  style={{ background: tone, position: "relative" }}
                >
                  {g.image ? (
                    <img
                      src={g.image}
                  ...
                  >
                    <span style={{ fontSize: 9, letterSpacing: "0.18em", color: "#1a1a1a" }}>
                      {g.name.toUpperCase()}
                    </span>
                  </div>
                  {badges[g.slug] && (
                    <span
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        go("/admin/photos?filter=missing");
                      }}
                      style={{
                        position: "absolute",
                        top: 8,
      ...
      {zoom && (
        <BohZoom
          ...
          snapshotUrl={
            PAGES[zoom.index].slug === "home"
              ? HOME_POSTER
              : (snapFor(PAGES[zoom.index].slug)?.url ?? null)
          }
          fromRect={zoom.rect}
          onClose={closeZoom}
          onNavigate={go}
...
function RibbonLink({
  ...
  return (
    <button
      onClick={onClick}
      className="boh-ribbon-link"
      style={{ color: accent ? T.attention : "inherit" }}
    >
      {text}
    </button>
  );
