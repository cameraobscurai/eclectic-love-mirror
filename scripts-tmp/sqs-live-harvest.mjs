...
for (const cat of CATS) {
  ...
  let r;
  try {
    r = await fetch(url, { headers: { "user-agent": "eh-mirror/1.0" } });
  } catch (e) {
    console.error("LIST-ERR", cat, e.message);
    continue;
  }
  if (!r.ok) {
    console.error("LIST-FAIL", cat, r.status);
    continue;
  }
  ...
  for (const it of items) {
    ...
    try {
      ...
      if (!pr.ok) {
        console.error(" fail", slug, pr.status);
        continue;
      }
