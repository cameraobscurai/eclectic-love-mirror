...
async function listAll(bucket, prefix = "") {
  ...
  while (true) {
    ...
    if (error) {
      console.error(bucket, prefix, error.message);
      break;
    }
...
for (const s of summary) {
  console.log(
    `${s.bucket.padEnd(22)} ${String(s.count).padStart(5)} files   ${String(s.mb).padStart(8)} MB`,
  );
  const top = Object.entries(s.folders)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 8);
...
console.log(
  `TOTAL`.padEnd(22) +
    String(grandCount).padStart(5) +
    ` files   ${(grandBytes / 1024 / 1024).toFixed(1)} MB`,
);
