...
for (const row of mirror.rows) {
  ...
  if (bad.length)
    affectedRows.push({ rms_id: row.rms_id, before: row.before, after: row.after, bad });
}
...
const { data: dbRows, error } = await sb
  ...
  .select("rms_id,title,slug,category,images")
  .in("rms_id", rmsIds);
...
for (const [feed, cats] of Object.entries(FEEDS)) {
  ...
  if (existsSync(cachePath)) {
    ...
    if (!r.ok) {
      console.warn(`    HTTP ${r.status} for ${feed}`);
      continue;
    }
    json = await r.json();
...
for (const row of affectedRows) {
  ...
  const scored = pool
    .map((f) => {
      let s = 0;
      for (const t of ptok) if (f.tokens.has(t)) s++;
      return { f, s };
    })
    .filter((x) => x.s >= 2)
    .sort((a, b) => b.s - a.s);
  ...
  if (!scored.length) {
    proposals.NONE.push({ rms_id: db.rms_id, title: db.title, category: db.category });
    continue;
  }
...
writeFileSync(
  "scripts-tmp/placeholder-rebind.json",
  JSON.stringify(
    {
