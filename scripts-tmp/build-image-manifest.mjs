...
for (const sp of siteProducts) {
  ...
  for (const m of r.matches) {
    ...
    if (
      !existing ||
      Math.abs(m.key.length - sp.title.length) <
        Math.abs(m.key.length - (existing.site_title || "").length)
    ) {
      PASS_B.set(m.rms_id, {
...
console.log(
  `[pass B] bound rms=${PASS_B.size}  exact=${bExact} contains=${bContains} token=${bToken} variant_families=${variantFamilies} unmatched_site=${unmatchedSite.length}`,
);
...
for (const rms_id of allRms) {
  ...
  if (a && b) {
    ...
    if (aHigh) {
      ...
      if (b.confidence === "exact" || b.confidence === "contains") {
        final.set(rms_id, {
          ...
          primary: bImages[0],
          gallery: bImages.slice(1),
          all_images: bImages,
          site_title: b.site_title,
          notes: "squarespace over token_fallback filename",
        });
      } else {
        // both weak -> flag
        conflicts.push({
          rms_id,
          reason: "both passes low confidence",
          a_paths: a.paths,
          b_urls: bImages,
          site_title: b.site_title,
        });
    ...
    final.set(rms_id, {
      source: "filename",
      confidence: a.confidence,
      primary: a.public_urls[0],
      gallery: a.public_urls.slice(1),
      all_images: a.public_urls,
    ...
    final.set(rms_id, {
      source: "squarespace",
      confidence: b.confidence,
      primary: b.image_urls[0],
      gallery: b.image_urls.slice(1),
      all_images: b.image_urls,
      site_title: b.site_title,
      notes:
        b.variant_family_size > 1
          ? `variant family of ${b.variant_family_size}`
          : "squarespace only",
    });
  }
}
...
const manifest = {
  ...
  counts: {
    ...
    pass_a: {
      bound_rms: PASS_A.size,
      exact: aExact,
      contains: aContains,
      token_fallback: aToken,
      storage_files_bound: filenamePassCoveredFiles,
    },
    pass_b: {
      bound_rms: PASS_B.size,
      exact: bExact,
      contains: bContains,
      token: bToken,
      variant_families: variantFamilies,
      unmatched_site: unmatchedSite.length,
    },
    final: {
...
const autoApplyExamples = sample(
  [...final.entries()].filter(([, v]) => v.source !== "squarespace" || v.confidence !== "token"),
  5,
);
...
lines.push(
  `1. Filename-key pass (storage files): ${filenamePassCoveredFiles} of ${storagePaths.length} bound`,
);
...
lines.push("");
lines.push(
  `2. Squarespace pass (site products): ${PASS_B.size} RMS rows bound from ${siteProducts.length} site products`,
);
...
lines.push("");
lines.push(
  `3. COMBINED COVERAGE: ${publicCovered} of ${publicRows.length} public RMS rows have ≥1 image`,
);
...
for (const [rms_id, v] of autoApplyExamples) {
  ...
  lines.push(
    `  ${rms_id}  "${x?.name}"  source=${v.source}  conf=${v.confidence}  imgs=${v.all_images.length}`,
  );
