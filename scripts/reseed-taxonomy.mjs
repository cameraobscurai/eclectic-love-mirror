...
for (const r of rows) {
  ...
  const base = {
    ...
    confidence: String(r.confidence ?? "high")
      .trim()
      .toLowerCase(),
    source: String(r.source ?? "squarespace")
      .trim()
      .toLowerCase(),
    title: String(r.title ?? ""),
  };
  ...
  for (const m of families[rmsId]?.members ?? []) {
    ...
    if (!assignments.has(id))
      assignments.set(id, { ...base, via: `family:${rmsId}`, title: m.title ?? base.title });
  }
}
...
if (rejects.length) {
  ...
  for (const r of rejects.slice(0, 40))
    console.error(`  ${r.rmsId} ${r.title} -> "${r.c}" / "${r.k}"`);
}
if (crossings.length) {
  console.log(`\n${crossings.length} collection-crossing correction(s) — declared tree wins:`);
  for (const m of crossings.slice(0, 40)) {
    console.log(
      `  ${m.rmsId} ${m.title}: workbook "${m.workbookCollection}" -> applied "${m.appliedCollection}/${m.category}"`,
    );
  }
}
if (BLOCKED && APPLY) {
...
if (EXPORT_CSV && fs.existsSync(EXPORT_CSV)) {
  ...
  for (const rec of recs.slice(1)) {
    ...
    if (!exportByTitle.has(title)) {
      exportByTitle.set(title, category);
      exportUsed += 1;
    }
  }
}
...
for (const [rmsId, row] of db) {
  ...
  if (row.category_slug || row.collection_slug)
    b4.push({ rmsId, title: row.title, c: row.collection_slug, k: row.category_slug });
}
...
const EXTRA_HINTS = {
  ...
  settee: "sofas-loveseats",
  sofa: "sofas-loveseats",
  loveseat: "sofas-loveseats",
...
for (const [tok, slug] of Object.entries(EXTRA_HINTS)) {
  if (bySlug.has(slug)) {
    if (!hintByToken.has(tok)) hintByToken.set(tok, new Set());
    hintByToken.get(tok).add(slug);
  }
}
const lint = [];
for (const [rmsId, a] of assignments) {
  ...
  if (suggested.size && !suggested.has(a.category_slug)) {
    lint.push({
      rmsId,
      title: a.title,
      assigned: a.category_slug,
      suggests: [...suggested].join(", "),
    });
  }
}
...
const table = (rowsArr, cols) =>
  [
    "| " + cols.join(" | ") + " |",
    "| " + cols.map(() => "---").join(" | ") + " |",
    ...rowsArr.map((r) => "| " + cols.map((c) => String(r[c] ?? "")).join(" | ") + " |"),
  ].join("\n");

fs.mkdirSync("docs", { recursive: true });
fs.writeFileSync(
  "docs/taxonomy-reseed-blockers.md",
...
${
  rejects.length
    ? table(
        rejects.map((r) => ({ rmsId: r.rmsId, title: r.title, collection: r.c, category: r.k })),
        ["rmsId", "title", "collection", "category"],
      )
    : "_None._"
}
`,
...
${table(
  Object.entries(byConfidence).map(([confidence, count]) => ({ confidence, count })),
  ["confidence", "count"],
)}
...
fs.writeFileSync(
  "docs/taxonomy-bucket4.md",
  `# Bucket 4 — assigned in db, absent from the v4 workbook
...
`;
let openQText = fs.existsSync(openQ)
  ? fs.readFileSync(openQ, "utf8")
...
console.log(
  `crossings ${crossings.length} · blanks ${blanks.length} · demotions ${demotions.length} · title lint ${lint.length}`,
);
...
for (const [rmsId, a] of assignments) {
  ...
  const review = demoted.has(rmsId)
    ...
    : {
        ...
        source: a.via === "reviewed" ? a.source : "family-inherit",
        reviewed: false,
        needs_owner: false,
      };
...
for (const r of b4) {
  const { error } = await sb
    ...
    .eq("rms_id", r.rmsId);
  if (error) {
    console.error("stamp error", r.rmsId, error);
    process.exit(1);
  }
  stamped += 1;
}
...
for (let from = 0; ; from += 1000) {
  const { data, error } = await sb
    .from("inventory_items")
    .select("collection_slug, taxonomy_review")
    .not("rms_id", "is", null)
    .range(from, from + 999);
  if (error) {
    console.error(error);
  ...
  for (const row of data) {
    if (row.collection_slug)
      actualByCollection[row.collection_slug] = (actualByCollection[row.collection_slug] ?? 0) + 1;
...
const mismatchLines = [];
if (written !== prediction.rowsWritten)
  mismatchLines.push(`rowsWritten ${written} != predicted ${prediction.rowsWritten}`);
if (stamped !== prediction.bucket4ReviewStamped)
  mismatchLines.push(`bucket4 ${stamped} != predicted ${prediction.bucket4ReviewStamped}`);
for (const [k, v] of Object.entries(prediction.perCollection)) {
  if ((actualByCollection[k] ?? 0) !== v)
    mismatchLines.push(`collection ${k}: actual ${actualByCollection[k] ?? 0} != predicted ${v}`);
}
for (const [k, v] of Object.entries(prediction.confidence)) {
  if ((actualConfidence[k] ?? 0) !== v)
    mismatchLines.push(`confidence ${k}: actual ${actualConfidence[k] ?? 0} != predicted ${v}`);
}
console.log("\n── actual vs predicted ──");
