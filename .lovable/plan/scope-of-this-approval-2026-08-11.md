Reply "approved — build Task 1.1" and hold it to the contract: the next output should be a SQL migration and regenerated types, nothing else. If it comes back with a migration _plus_ opportunistic edits to ProductTile or the fit files, that's scope creep against the freeze — reject the extras, keep the migration. One task per prompt, each closed by its Done-when, is the discipline that gets Phase 1 landed in a day instead of a week.
