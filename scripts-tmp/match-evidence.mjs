...
function distinctiveTokens(s) {
  return rawTokens(s).filter(
    (t) => t.length >= 3 && !STOP.has(t) && !FORM_FACTORS.has(t) && !SIZE_RX.test(t),
  );
}
...
function scorePair(lc, dc, liveCat) {
  ...
  if (sharedTok.length >= 2) {
    pts += sharedTok.length;
    ev.shared_tokens = sharedTok;
  ...
  if (typeof dc.row.quantity === "number") {
    ...
    if (qm && Number(qm[1]) === dc.row.quantity) {
      pts += 2;
      ev.qty_match = dc.row.quantity;
    }
  }

  // dimensions
  const sharedDims = [...lc.dims].filter((d) => dc.dims.has(d));
  if (sharedDims.length >= 2) {
    pts += 3;
    ev.shared_dims = sharedDims;
  } else if (sharedDims.length === 1) {
    pts += 1;
    ev.shared_dim = sharedDims[0];
  }
...
for (const [slug, top] of liveTop) {
  ...
  const rec = {
    ...
    db_margin: dbMargin,
    runner_up: runner
      ? { rms_id: runner.rms_id, db_title: runner.db_title, pts: runner.pts }
      : null,
    evidence: best.ev,
  };
...
for (const [rms_id, b] of bindings) {
  ...
  matchBound.push({
    rms_id,
    db_title: dc.row.title,
    db_category: dc.row.category,
    live_slug: b.live_slug,
    live_title: lc.p.title,
    live_category: lc.p.categories[0],
    wave: b.wave,
    evidence: b.evidence,
    pts: b.pts,
  ...
  JSON.stringify(
    {
      ...
      by_wave: matchBound.reduce((a, m) => {
        a[m.wave] = (a[m.wave] || 0) + 1;
        return a;
      }, {}),
      bindings: matchBound,
    },
...
// CSV: fuzzy review
const csvHead =
  "live_slug|live_title|live_cat|rms_id|db_title|db_cat|pts|margin|mutual|db_margin|runner_rms|runner_title|runner_pts|evidence";
...
for (const [slug, lc] of liveCache) {
  ...
  unmatched.push({
    slug,
    title: lc.p.title,
    cat: lc.p.categories[0],
    n_images: lc.canonImages.size,
  });
}
writeFileSync(
  "scripts-tmp/match-unmatched.csv",
  [
    "slug|title|cat|n_images",
    ...unmatched.map((u) => `${u.slug}|${u.title}|${u.cat}|${u.n_images}`),
  ].join("\n"),
);

// summary
const byWave = matchBound.reduce((a, m) => {
  a[m.wave] = (a[m.wave] || 0) + 1;
  return a;
}, {});
const summary = [
