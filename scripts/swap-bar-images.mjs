...
const MAP = [
  ...
  {
    file: 3,
    id: "7268b08d-7423-47f5-ab5c-d24763ddb649",
    slug: "arcus-tavion-12-natural-tambour-bar",
  },
...
for (const m of MAP) {
  ...
  if (up.error) {
    console.error("UPLOAD FAIL", m.slug, up.error);
    process.exit(1);
  }
  ...
  const { data: row, error: ge } = await sb
    .from("inventory_items")
    .select("images")
    .eq("id", m.id)
    .single();
  if (ge) {
    console.error("GET FAIL", m.slug, ge);
    process.exit(1);
  }
  ...
  if (ue) {
    console.error("UPDATE FAIL", m.slug, ue);
    process.exit(1);
  }
