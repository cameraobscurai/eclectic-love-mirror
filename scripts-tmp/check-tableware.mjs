...
for (const p of tw)
  for (const i of p.images || [])
    if (i.url) urls.push({ slug: p.slug, title: p.title, url: i.url, hero: i.isHero });
...
async function w() {
  while (q.length) {
    ...
    if (!j) return;
    try {
      const r = await fetch(j.url, { method: "HEAD" });
      if (r.ok) ok++;
      else bad.push({ ...j, status: r.status });
    } catch (e) {
      bad.push({ ...j, err: String(e.message) });
    }
  }
}
...
console.log("first 15 broken:");
for (const b of bad.slice(0, 15))
  console.log(" -", b.hero ? "HERO" : "    ", b.status || b.err, b.slug, "<-", b.url);
