...
const RULES = {
  ...
  tables: [
    {
      id: "dining-tables",
      kws: ["dining table", "farm table", "community table", "counter table"],
    },
...
for (const [liveSlug, info] of Object.entries(live.byLiveSlug)) {
  ...
  rows.push({
    ...
    liveSubs: info.subcategories,
    ourSlug: our?.slug,
    ourTitle: our?.title,
    ourCat: our?.categorySlug,
    derivedSub: our ? deriveSub(our) : null,
    how,
  });
}
...
for (const m of trueMismatches.slice(0, 40)) {
  console.log(
    `  · ${m.title.padEnd(45)} live=${m.live.padEnd(20)} expected=${(m.expected || "-").padEnd(15)} got=${m.got || "(none)"}`,
  );
}

writeFileSync(
  "scripts/audit/live-vs-runtime-subcat-diff.json",
  JSON.stringify(
    {
      summary: {
        totalLive: rows.length,
        missing: missing.length,
        trueMismatches: trueMismatches.length,
      },
      trueMismatches,
      missing,
    },
