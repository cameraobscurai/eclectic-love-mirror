...
for (const url of URLS) {
  ...
  console.log(
    `  title="${parsed.product.product_title_normalized}" dims="${parsed.product.dimensions}" qty="${parsed.product.stocked_quantity}"`,
  );
  console.log(
    `  images=${parsed.images.length} confidence=${parsed.product.parse_confidence.toFixed(2)} reextract=${parsed.product.needs_llm_reextract} (${parsed.product.reextract_reason})`,
  );
}
