...
export const ScenePalette: React.FC = () => {
  ...
  // Reveal helpers
  const revealOp = (at: number) =>
    interpolate(frame, [at, at + 14], [0, 1], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    });
  const revealY = (at: number) =>
    interpolate(frame, [at, at + 14], [10, 0], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    });

  const label1Op = revealOp(0);
  const div1Op = interpolate(frame, [BUTTON_AT - 6, BUTTON_AT + 6], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const div2Op = interpolate(frame, [FIRST_SWATCH - 10, FIRST_SWATCH], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  const label2Op = div2Op;
  const div3Op = interpolate(frame, [CLOSER_AT - 8, CLOSER_AT + 6], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });
  ...
  return (
    <AbsoluteFill>
      <IndexCard
        step={3}
        label="Palette"
        subtitle="Pulled from everything you chose."
        sceneLen={SCENE_LEN}
      >
        {/* PULLED FROM label */}
        <div
        ...
        {REAL_PALETTE.map((sw, i) => {
          ...
          const t = interpolate(frame, [startFrame - 6, startFrame + 14], [0, 1], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          });
          ...
          const labelOp = interpolate(frame, [startFrame + 8, startFrame + 20], [0, 1], {
            extrapolateLeft: "clamp",
            extrapolateRight: "clamp",
          });
