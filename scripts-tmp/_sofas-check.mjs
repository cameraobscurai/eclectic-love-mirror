...
const missed = items.filter(
  (p) =>
    /sofa|loveseat|settee|couch|sectional/i.test(p.title || p.name || "") &&
    getProductBrowseGroup(p) !== "sofas",
);
...
console.log(
  missed
    .map(
      (p) =>
        `  ${getProductBrowseGroup(p)} <- ${p.title || p.name} [${p.category || p.categorySlug}]`,
    )
    .join("\n"),
);
