...
import { physicalScaleFor, parseDimensionsInches } from "@/components/collection/productPhysicalScale";
...
for (const p of prods) {
  ...
  console.log([p.title.padEnd(38), (p.categorySlug+'/'+(p.liveSubcategories?.[0]??p.subcategory??'-')).padEnd(28), d?`${d.width}x${d.height}`:'NO-DIMS', 'size='+s.size.toFixed(2), 'area='+(f.primary==='area'?f.primaryTarget.toFixed(2):f.primary+':'+f.primaryTarget.toFixed(2)), 'hMax='+(f.heightMax?.toFixed(2)??'-'), 'wMax='+(f.widthMax?.toFixed(2)??'-')].join(' | '));
}
